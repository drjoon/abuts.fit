...
public class Connect : IDTExtensibility2
{
    ...
    private static Esprit.Application _EspritApp;
    ...
    private Esprit.ToolBar TBar;

    private EspritMenus.Menu MyMenu;
    ...
    public static Esprit.Application EspritApp
    {
        [CompilerGenerated]
        get
        {
        ...
        set
        {
            ...
            Esprit.Application espritApp = _EspritApp;
            if (espritApp != null)
            {
    ...
    private bool CheckLicense(string ctmcode, string licnumber)
    {
        ...
        try
        {
            ...
            if (File.Exists(EspritApp.Path + "AddIns\\DentalAddin\\" + licnumber + ".Lic"))
            {
                ...
                bool flag = Operators.CompareString(text.Split('|')[0].ToLower(), ctmcode.ToLower(), false) == 0;
                bool flag2 = Operators.CompareString(text.Split('|')[1].ToLower(), licnumber.ToLower(), false) == 0;
    ...
    public void OnConnection(object CallingApplication, ext_ConnectMode ConnectMode, object AddInInst, ref Array custom)
    {
        ...
        EspritApp = (Esprit.Application)CallingApplication;
        ...
        checked
        {
            try
            {
                ...
                while (num <= 1);
                MyMenu = (EspritMenus.Menu)NewLateBinding.LateGet(EspritApp.Menus, (Type)null, "item", new object[1] { 3 }, (string[])null, (Type[])null, (bool[])null);
                int count = MyMenu.Count;
                for (num = 1; num <= count; num++)
                {
                    EspritMenus.MenuItem menuItem = MyMenu[num];
                    ...
                    {
                        EspritMenus.Menu subMenu = menuItem.SubMenu;
    ...
    private void AddIn_OnCommand(int Cookie, int UserId)
    {
        ...
        switch (UserId)
        {
            case 0:
                ...
                break;
            case 1:
                if (new Dialog1().ShowDialog() == DialogResult.OK)
                {
                    AddTab.Init();
                }
    ...
    private void ShowTB()
    {
        foreach (Esprit.ToolBar toolBar in EspritApp.ToolBars)
        {
    ...
    private void CleanUp()
    {
        checked
        {
            try
            {
                ...
                {
                    EspritMenus.MenuItem menuItem = MyMenu[i];
                    if ((Operators.CompareString(menuItem.Name, "&工具栏...", false) == 0) | (Operators.CompareString(menuItem.Name, "&Toolbars...", false) == 0) | (Operators.CompareString(menuItem.Name, "툴바(&T)", false) == 0))
                    {
                        EspritMenus.Menu subMenu = menuItem.SubMenu;
                        try
