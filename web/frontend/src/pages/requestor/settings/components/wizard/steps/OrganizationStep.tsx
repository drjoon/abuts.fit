...
import type { LicenseStatus } from "../../business/types";
  ...
  const renderOwnerForm = () => (
    <div className="space-y-5">
      <p className="text-sm text-slate-500">
        사업자 기본 정보를 입력하면 검증 후 자동으로 대표 기공소가 생성돼요.
      </p>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {OWNER_ALL_FIELDS.map(({ key, label, placeholder, span }) => {
          ...
          return (
            <div className={cn("space-y-2", span)} key={key}>
              <Label>
                {label}
                {ownerErrors[key] && (
              ...
              <Input
                ...
                onChange={(e) => {
                  setOwnerForm((prev) => ({ ...prev, [key]: e.target.value }));
                  if (ownerErrors[key]) {
                    setOwnerErrors((prev) => ({ ...prev, [key]: "" }));
                  }
                }}
                className={ownerErrors[key] ? "border-destructive" : ""}
              />
            </div>
          );
        })}
      ...
      <div className="flex justify-end">
        <Button
          ...
          disabled={saving}
        >
          {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "완료"}
        </Button>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <Button
          ...
          variant="default"
          onClick={handleJoin}
          disabled={!selectedOrgId || joinLoading}
          className="w-20 h-11"
        >
          {joinLoading ? (
