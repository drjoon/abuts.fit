...
export const SettingsWizard = ({
  ...
  const [currentStep, setCurrentStep] = useState<WizardStepId>("profile");
  ...
  const applyProgressSteps = useCallback((steps?: GuideProgressStep[]) => {
    if (!Array.isArray(steps)) return;
    setStepCompleted(() => {
      const next = createStepCompletionState();
      steps.forEach((entry) => {
        const stepId = GUIDE_TO_STEP[entry.stepId];
        if (stepId) {
          next[stepId] = entry.status === "done";
        }
      });
      return next;
    });
  ...
  useEffect(() => {
    ...
    applyProgressSteps(progress.steps);
  }, [applyProgressSteps, progress?.steps]);
  ...
  const handleNext = useCallback(async () => {
    if (currentStep === "profile" || currentStep === "phone") {
  ...
  const handlePrev = useCallback(() => {
    ...
    void markGuideStep(currentStep, false);
  }, [currentStep, markGuideStep]);
