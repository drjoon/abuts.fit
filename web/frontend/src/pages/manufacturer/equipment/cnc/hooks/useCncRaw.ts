...
export const useCncRaw = () => {
  ...
  // 중요: callRaw 참조가 매 렌더마다 새로 만들어지면, 이를 deps에 포함하는
  // 컨슈머 훅(useCncToolSlots 등)의 useCallback / useEffect 가 무한 재실행되어
  // 백엔드에 라우 호출이 폭주(→ 429 rate-limited)할 수 있다.
  // 따라서 token 이 바뀌지 않는 한 동일 함수 참조를 유지하도록 useCallback 으로 감싼다.
