...
const offsetToRevealSelected = (
  ...
  if (indices.length === 0) return toothChartOffsetForRegion("M", teeth.length);
...
const initialToothChartOffsets = (selected: ReadonlySet<string>) => {
  const next: Record<string, number> = {};
  for (const decade of TOOTH_CHART_ROWS) {
    next[decade.key] = offsetToRevealSelected(decade.teeth, selected);
  }
  return next;
};
