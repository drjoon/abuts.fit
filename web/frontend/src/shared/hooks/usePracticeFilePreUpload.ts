...
export const toPracticeUploadFileKey = (file: File) =>
 ...
 */
export function usePracticeFilePreUpload(options: Options) {
  const { token } = options;
  ...
  const forgetFile = useCallback(
    (file: File) => {
      forgetFileKey(toPracticeUploadFileKey(file));
    },
    [forgetFileKey],
  );
  ...
  const ensureFilesUploaded = useCallback(
    async (
      ...
      for (let i = 0; i < files.length; i += 1) {
        const key = toPracticeUploadFileKey(files[i]);
      ...
      if (pendingIndexes.length > 0) {
        ...
        pendingIndexes.forEach((fileIndex, batchIndex) => {
          ...
          const key = toPracticeUploadFileKey(file);
        ...
        pendingIndexes.forEach((fileIndex, batchIndex) => {
          ...
          progressMap[progressKey] = 100;
          cacheRef.current.set(toPracticeUploadFileKey(files[fileIndex]), {
            status: "done",
            result: row,
          });
        });
  ...
  const preUploadFiles = useCallback(
    (files: File[]) => {
      ...
      const need = files.filter((file) => {
        const entry = cacheRef.current.get(toPracticeUploadFileKey(file));
