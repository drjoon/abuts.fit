...
- practice 최근 전송 기공소(기공소 선택 드롭다운) SSOT:
  ...
  - 기공의뢰 상단 툴바: 「새로 작성」·「최근 의뢰」·「임시저장」·「휴지통」. 최근 의뢰=전체보기 모달(`PracticeRecentTransfersAllModal`: 헤더에 상태 6뱃지+검색, 3주 세로 스크롤 캘린더, 년/월 오른쪽 요일 숨김, 칸 날짜=월/일, 기공소 정렬·색). 임시저장/휴지통=다이얼로그. 우측 목록 없이 작성 폼 전폭(치식 full 차트, 카드 min-w로 어벗 라벨 표시). 본문 순서: 기공소·환자·일정 → **메모|드롭존 2열** → 보철물.
