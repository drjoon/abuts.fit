...
- practice
  ...
  - `src/shared/components/practice/PracticeTransferRequestIntakePanel.tsx`
    ...
    - 기공의뢰서 상·하악 사이(크게보기·전송 상세 포함)에 견적(크레딧 소비액) 표시. 치과는 평소 블러, 호버 시 금액 공개. **자동매칭**은 최소 별(2~5)에 따른 플랫폼 고정수가(평균×배수) 견적·청구. 간단 합계 + 빠른툴팁 치식별 세부(컬럼: 기공소 기공물 / 기공소 어벗 / 어벗츠 어벗). 환봉 요청중은 기공소 어벗에 `요청중`(또는 기공소 커스텀어벗 수가). `PracticeTransferFeeEstimate` / `GET /api/practice/transfers/quote-context`.
