...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 안내 모달([정책 안내])의 리메이크: **치과로부터(PTX) = 무료**, **어벗츠로(Request) = 건당 10,000원**. 동일 치과·환자·치식·최근 90일 조건 충족 시 적용(월 무료 쿼터 없음).
  - 관련 파일:
    - `src/shared/ui/PricingPolicyDialog.tsx`
    - `src/pages/requestor/dashboard/components/RequestorPolicyRemakeHeader.tsx`
...
- 부가세(VAT) / 면세 UI 정책(강제, 루트 `rules.md` §2.3) — 겸영 이중 체계:
  ...
  - **스토어 기성품(과세)**: 고객 표시는 **부가세 포함가** + `과세 · 부가세 포함`. 유료 크레딧(`CHARGE_PAID`) 누적 충전 ≥550만이면 **패키지 구매자** — 판매가 취소선 + pkg가. 주문·카탈로그 API도 동일 단가. 크레딧(유료) 결제. 한 체크아웃에 기공과 합치지 않음. `RequestorStorePage` / cart / orders. 헬퍼: `shared/tax/invoiceLabels.ts` · `shared/store/storeCatalog.ts`.
...
- practice 전송 상태 표준(치과/의뢰자 공통): `발송완료 | 취소 | 수신완료 | 의뢰수락 | 자동매칭 | 작업완료 | 생산진행`
  ...
  - **PTX 리메이크**: 목록「리메이크」확인은 **커스텀어벗 기본 제외**(체크 옵트인). 포함 시 기공소→어벗츠는 동일 90일 조건에서 건당 10,000원. 치과→기공소 리메이크비는 원본 90일 이내면 무료(`LAB_FEE_REMAKE_FREE`). 목록 `remakeFeeQuote`=보철만, `remakeFeeQuoteWithCustomAbutment`=CA 포함.
