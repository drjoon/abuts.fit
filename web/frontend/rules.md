...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    - 내역 상단 요약은 **수식 카드**(`현재 잔액 = 유료 충전 + 무료 충전 [+ 정산 적립] − 소비`). 연산 부호만 카드 사이, 설명은 툴팁.
    ...
    - **치과**: 유료크레딧(+무료·의뢰/배송)만 표시. `기공크레딧` 잔액 행·정산 필터 숨김. **기간 합계**·드릴다운은 필터 줄 **오른쪽** `PeriodFilter`(기간 전용 state). 내역 테이블은 기간 무관. API `from`/`to`는 `appendPeriodQueryParams`(KST YMD SSOT). 데모 모드면 잔고 힌트·뱃지만(무료 카드 라벨은 「무료 충전」).
    ...
    - **통계 탭**(`CreditStatisticsTab`): 치과는 `유료 + 무료 − 소비 | 의뢰건수`. 기공소는 **치과로부터 수신**(정산 적립·치과별·보철)과 **어벗츠로 의뢰**(충전·소비·유형)를 구역 분리. 파트너 패널=`치과별 적립`(보류 포함, 정산만). 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
