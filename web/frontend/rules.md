...
- 관리자 크레딧
  ...
  - `src/features/settings/tabs/AdminCreditSettingsTab.tsx` (`variant=credits`: 수동 무료크레딧 기본액·배송 / `variant=customAbut`: 고시 생산·디자인+생산·딜러없음 분배·레거시 기공소 오버레이·환봉 추가요청)
...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    ...
    - **치과**: 유료크레딧만 표시(잔여 무료>0일 때만 보조). `기공크레딧` 잔액 행·정산 필터 숨김. **기간 합계**·드릴다운은 필터 줄 **오른쪽** `PeriodFilter`(기간 전용 state). 내역 테이블은 기간 무관. API `from`/`to`는 `appendPeriodQueryParams`(KST YMD SSOT). 충전 카드 라벨「충전」. 데모 모드면 잔고 힌트·뱃지「데모 N일 남음」.
    - **기공소**: 치과와 동일 수식 카드 + **정산 적립**(확정만 · 적립 보류 제외). 기간 필터는 카드만(테이블 기간 무관). 버킷 필터(유료/무료/기공)·동작(충전/소비/조정) 유지. `SPEND_SETTLEMENT`=기공크레딧 주문 사용.
    - **기공소 기공의뢰수신**: 의뢰카드·전송 상세에 수령액(청구 − 플랫폼 수수료) 표시.
    - **기공소 기공의뢰 장부**: 내역 행(지급상태·금액 호버·상세)은 **치과와 동일**. 보철기공비와 어벗 디자인+생산비는 모두 기공비. 금액 호버는 두 열(+배송비)·기공비 총액까지. 기공소→치과·치과→기공소 배송은 무료(기공수가 배송 항목 없음). 지급 상태는 **적립 보류/일부 적립/적립 완료**(치과는 결제 보류·완료). 커스텀어벗은 디자인 STL 업로드 후 어벗츠에 생산비가 지급될 때까지 정산·지급에서 빠지며, 기간이 지나도 그때 정산된다(정책 안내·정산규칙·정산 적립 툴팁). lab-share HOLD는 기공크레딧 잔액·상단 정산 적립 합계에 넣지 않고 「적립 보류」행으로만 미러. 행 클릭 상세는 치과와 같은 견적 뷰(배송비·크레딧 소비 총액 포함). CA 디자인비 원장(`abutment_design_lab_fee`)을 생산의뢰 행으로 따로 표시하지 않는다.
