...
- 부가세(VAT) / 면세 UI 정책(강제, 루트 `rules.md` §2.3) — 겸영 이중 체계:
  ...
  - **스토어 기성품(과세)**: 고객 표시는 **부가세 포함가** + `과세 · 부가세 포함`. 장바구니·입금주문·세금계산서. 커스텀어벗·크레딧과 합치지 않음. `RequestorStorePage` / cart / orders. 헬퍼: `shared/tax/invoiceLabels.ts`.
  - **관리자 스토어**: 재고·주문 승인 `/dashboard/store-admin` (`AdminStorePage`).
