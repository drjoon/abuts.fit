...
- 요약 카드 (`SettlementStatCard`, 정산·크레딧 상단 요약):
  ...
  - 통계(치과): `충전 − 소비 | 의뢰건수`.
  - 통계(기공소): `치과로부터 수신`=`정산 적립 | 의뢰건수`, `어벗츠로 의뢰`=`충전 − 소비 | 의뢰건수`.
...
- practice
  ...
  - `src/shared/components/practice/PracticeTransferRequestIntakePanel.tsx`
    ...
    - 기공소 의뢰카드·전송 상세: 주 표기(합산 라벨)=`기공비`·금액=보철기공비+어벗 디자인+생산비. 하청이면 보조로 수령=`기공비×(1−subcontractFeeRate)`. 원청(어벗츠 기공사업부)이 하청을 준 뒤 자기 화면은 전액 수주(수수료 0). 지정·협력은 이벤트 중 전액(지정 수수료 기본 off · 정책 2%). 툴팁 트리거는「기공비」텍스트만(우측 ? 제외). 목록 `feeQuote` SSOT.
...
- 개발·운영사 설정
  ...
  - 의뢰자(치과) 설정: 구독 탭 없음. 구 `?tab=subscription` → 계정. 대시보드 헤더는 `[정책 안내]`만.
    ...
    - `src/shared/ui/PricingPolicyDialog.tsx` — 런칭 이벤트 1만(+배송) / 정상가 1.3만(+배송 또는 FM덴탈 월정액·기공소만). 신속 +2,000 · 배송 3,500. FM 가입·해지(대표·기공소). 기공소·딜러·영업팀 `requestor`: 플랫폼 사용료 관리자 %(초기 2%·이벤트 off UI `~~2%~~ → 0%`) · 하청 10%. **딜러(`variant=salesman`)**: 심플웨이·커스텀어벗 매출(기공 제외) 20% · 90일 무주문이면 소개 리셋. **「의뢰자 정책」(`variant=requestor`)**: 딜러 대시보드·영업팀 성과 — 소개 치과·기공소 안내용 단가·출고·기공소 수수료.
  ...
  - 기공의뢰 자동매칭: `PracticeTransferAutoMatchTab`(카드·탭 **인증 기공소**) — 수수료 스트립(하청%/지정 on·off·%) + 기공소별 인증 ON·기공 테스트·메모. 기공소 설정 탭은 없음. 관리자 테스트 통과/`enabled` 시 풀 참여. 지정·협력 플랫폼 사용료 정책 **2%** · 이벤트 기본 **off**(`directPlatformFeeEnabled` / `directPlatformFeeRate`). 하청 `subcontractFeeRate`(기본 10%). 관리자 플랫폼 설정「인증 기공소」탭
...
- 신규의뢰 첨부·구강스캔 묶음 (파일 크기 SSOT):
  ...
  - **3MB 초과 드롭/파일 오픈**: 첨부 전 `ConfirmDialog`로 확인. 3D 프리뷰(`StlPreviewViewer`) + «커스텀 어벗 STL만 받음» + 구강스캔이면 치과=`구강스캔으로`·기공소=`치과로부터 수신` 안내. 확인 시에만 어벗생산의뢰로 첨부. 치과 링크 클릭 시 `navigate(..., { state: { prefilledFiles } })`로 파일을 넘기고 `PracticeFileTransferPage`가 첨부(기공소는 수신 페이지로만 이동).
  - 자동 분류·묶음 (3MB 단일 기준):
...
- 신규의뢰 배송 방식(묶음/신속):
  ...
  - 디자인+생산(`design_custom_abutment`): `(생산 단가 + 디자인비) × 어벗 수`.
    ...
    - 안내·청구 정가 SSOT (`creditSettings` + `src/shared/pricing/abutsAbutmentService.ts`): 정상가 **1.3만원** · 런칭 **1만원**(`resolveCustomAbutmentProductionPriceForAt`). 신속 `expressFee`(기본 +2,000). 배송 박스당 또는(기공소) FM덴탈 월정액. `regular*`는 관리자 딜러분배용. 기공소 매칭 월정 0·지정/협력 플랫폼 사용료 정책 2%·이벤트 0% · 하청 10% — 루트 `rules.md` §2.3.
...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 기공소 어벗츠 인증: 관리자 `PracticeTransferAutoMatchTab`에서 신청·테스트·상태 관리. **월 참여 수수료 0원**(정책). 지정·협력 플랫폼 사용료는 정책 **2%** · 이벤트 기간 **0%**(`directPlatformFeeEnabled` 기본 off, 추후 공지 후 on). 하청 `subcontractFeeRate`(기본 10%). 구 거래 치과 소개 UI는 제거(초대 API는 레거시 유지). 구 기공소 설정「어벗츠 인증」탭 제거.
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    ...
    - **통계 탭**(`CreditStatisticsTab`): 치과는 `충전 − 소비 | 의뢰건수`(소비에 결제 보류 HOLD 포함). 기공소는 **치과로부터 수신**(정산 적립 확정·치과별·보철)과 **어벗츠로 의뢰**(충전 − 소비·유형)를 구역 분리. 파트너 패널=`치과별 적립`(확정만). 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
