...
- 역할별 정산
  ...
  - 제조사: `src/pages/manufacturer/payments/PaymentsPage.tsx` — 거래 원장(일시·지급상태·금액·잔액·거래내역). 유형 열은 생략(모두 커스텀어벗 생산+배송비). **생산·배송은 KST 하루 1행**(의뢰 1건=어벗 1개라 기공의뢰처럼 못 묶음). 클릭 상세는 의뢰/배송을 별 섹션으로 나누고, 그 안에서 **배송자 BA**(requestor businessAnchor·우편함)별. PTX는 치과명이 아니라 기공소 BA로 표기. 장부는 공급가(어벗 1개당 매입가 기본 8,800 부가세 포함→공급가). 리메이크·무료 크레딧 결제건은 무료 생산(지급 0). 유료 결제분만 미정산으로 쌓이며 말일 일괄 지급(지급 시 +VAT·세금계산서). 정산규칙 모달은 플랫폼 설정 매입가·배송단가를 표시.
...
- 부가세(VAT) / 면세 UI 정책(강제, 루트 `rules.md` §2.3) — 겸영 이중 체계:
  ...
  - 제조사 정산규칙: 원청–하청 고정 매입가(의뢰·배송, 부가세 포함)·장부 공급가·지급 시 VAT·세금계산서. 유료 결제건만 약정 단가(리메이크·무료 크레딧은 무료 생산) · 말일 일괄 지급. % 분배 안내 금지.
...
- 제조사 정산(일별) 표시 정책:
  - 하청 고정단가: **어벗 1개당** 매입가(기본 8,800 부가세 포함) / 배송 박스당 3,500(부가세 포함). 상단 카드·상세는 의뢰/배송을 **분리** 표시(공급가·건수). 리메이크·무료 크레딧 결제건은 지급 0. 유료만 지급 시 +VAT·세금계산서.
  ...
  - 제조사 지급 대상은 **약정 하청비 전액**(공급가 + 부가세 · 세금계산서). 리메이크·무료 크레딧 결제건은 제외하고 유료만 지급한다. 매달 말일 일괄 지급 전까지 상단「미정산」에 쌓인다. 지급 완료분은「지급 합계」.
