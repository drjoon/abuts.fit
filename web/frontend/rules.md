...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    - 내역 상단 요약은 **수식 카드**(`현재 잔액 = 유료 충전 + 데모/무료 충전 [+ 정산 적립] − 소비`). 연산 부호만 카드 사이, 설명은 툴팁.
    ...
    - **치과**: 유료크레딧(+무료·의뢰/배송)만 표시. `기공크레딧` 잔액 행·정산 필터 숨김. **기간 합계**·드릴다운은 필터 줄 **오른쪽** `PeriodFilter`(기간 전용 state). 내역 테이블은 기간 무관. API `from`/`to`는 `appendPeriodQueryParams`(KST YMD SSOT). 데모 모드면 무료 충전 카드 라벨=`데모 충전`.
    ...
    - **기공소 기공의뢰 장부**: 내역 행(지급상태·금액 호버·상세)은 **치과와 동일**. 보철기공비와 어벗 디자인+생산비는 모두 기공비. 금액 호버는 두 열(+배송비)·기공비 총액까지. 기공소→치과·치과→기공소 배송은 무료(기공수가 배송 항목 없음). 지급 상태는 **적립 보류/일부 적립/적립 완료**(치과는 결제 보류·완료). **작업완료 전** lab-share HOLD는 기공크레딧 잔액에 넣지 않고 「적립 보류」행으로 미러(치과 데모 크레딧이면 금액 아래「데모」). 행 클릭 상세는 치과와 같은 견적 뷰(배송비·크레딧 소비 총액 포함). CA 디자인비 원장(`abutment_design_lab_fee`)을 생산의뢰 행으로 따로 표시하지 않는다.
    - **통계 탭**(`CreditStatisticsTab`): 치과는 `유료 + 데모/무료 − 소비 | 의뢰건수`. 기공소는 **치과로부터 수신**(정산 적립·치과별·보철)과 **어벗츠로 의뢰**(충전·소비·유형)를 구역 분리. 파트너 패널=`치과별 적립`(보류 포함, 정산만). 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
