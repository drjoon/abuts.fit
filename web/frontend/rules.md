...
- 역할별 정산
  ...
  - 제조사: `src/pages/manufacturer/payments/PaymentsPage.tsx` — 거래 원장(일시·지급상태·금액·잔액·거래내역). 유형 열은 생략(모두 커스텀어벗 생산+배송비). **생산·배송은 KST 하루 1행**(의뢰 1건=어벗 1개라 기공의뢰처럼 못 묶음). 클릭 상세는 의뢰/배송을 별 섹션으로 나누고, 그 안에서 **배송자 BA**(requestor businessAnchor·우편함)별. PTX는 치과명이 아니라 기공소 BA로 표기. 장부·미정산=부가세 포함가(어벗 1개당 매입가 기본 8,800). 리메이크만 무료 생산(지급 0). 무료 크레딧 결제 포함 약정 단가가 미정산으로 쌓이며 말일 일괄 지급(재가산 없음·세금계산서÷1.1). 정산규칙 모달은 플랫폼 설정 매입가·배송단가를 표시.
...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    ...
    - **기공소**: 치과와 동일 수식 카드 + **정산 적립**(확정·적립 보류 합). 기간 필터는 카드만(테이블 기간 무관). 버킷 필터(유료/무료/기공)·동작(충전/소비/조정) 유지. `SPEND_SETTLEMENT`=기공크레딧 주문 사용.
    - **기공소 기공의뢰수신**: 의뢰카드·전송 상세에 수령액(청구 − 플랫폼 수수료) 표시.
    - **기공소 기공의뢰 장부**: 내역 행(지급상태·금액 호버·상세)은 **치과와 동일**. 보철기공비와 어벗 디자인+생산비는 모두 기공비. 금액 호버는 두 열(+배송비)·기공비 총액까지. 기공소→치과·치과→기공소 배송은 무료(기공수가 배송 항목 없음). 지급 상태는 **적립 보류/일부 적립/적립 완료**(치과는 결제 보류·완료). **작업완료 전** lab-share HOLD는 기공크레딧 잔액에 넣지 않고 「적립 보류」행으로 미러. 행 클릭 상세는 치과와 같은 견적 뷰(배송비·크레딧 소비 총액 포함). CA 디자인비 원장(`abutment_design_lab_fee`)을 생산의뢰 행으로 따로 표시하지 않는다.
    - **통계 탭**(`CreditStatisticsTab`): 치과는 `충전 − 소비 | 의뢰건수`. 기공소는 **치과로부터 수신**(정산 적립·치과별·보철)과 **어벗츠로 의뢰**(충전 − 소비·유형)를 구역 분리. 파트너 패널=`치과별 적립`(보류 포함, 정산만). 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
  ...
  - 안내 모달([정책 안내])의 리메이크 판정은 동일 `90일`(치과·환자·치식) 기준을 사용합니다. 리메이크 과금은 건당 10,000원(월 무료 쿼터 없음).
  - 관련 파일:
    - `src/shared/ui/PricingPolicyDialog.tsx`
    - `src/pages/requestor/dashboard/components/RequestorPolicyRemakeHeader.tsx`
...
- 부가세(VAT) / 면세 UI 정책(강제, 루트 `rules.md` §2.3) — 겸영 이중 체계:
  ...
  - 제조사 정산규칙: 원청–하청 고정 매입가(의뢰·배송, 부가세 포함)·장부·미정산 포함가·지급 재가산 없음·세금계산서. 리메이크만 무료 생산(0). 무료 크레딧 포함 약정 단가 전액 · 말일 일괄 지급. % 분배 안내 금지.
...
- 제조사 정산(일별) 표시 정책:
  - 하청 고정단가: **어벗 1개당** 매입가(기본 8,800 부가세 포함) / 배송 박스당 3,500(부가세 포함). 상단 카드·상세는 의뢰/배송을 **분리** 표시(포함가·건수; 푸터에 공급가 참고 가능). 리메이크만 지급 0. 무료 크레딧 포함 약정 단가 전액·지급 재가산 없음·세금계산서.
  ...
  - 제조사 지급 대상은 **약정 하청비 전액**(부가세 포함가 · 세금계산서÷1.1). 리메이크만 제외하고 무료 크레딧 포함 약정 단가를 지급한다. 매달 말일 일괄 지급 전까지 상단「미정산」에 쌓인다. 지급 완료분은「전월 지급」.
