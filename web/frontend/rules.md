...
- 요약 카드 (`SettlementStatCard`, 정산·크레딧 상단 요약):
  ...
  - 내역: `현재 잔액 = 유료 충전 + 무료 [+ 정산 적립] − 소비`.
  - 통계(치과): `유료 + 무료 − 소비 | 의뢰건수`.
  - 통계(기공소): `치과로부터 수신`=`정산 적립 | 의뢰건수`, `어벗츠로 의뢰`=`유료 + 무료 − 소비 | 의뢰건수`.
...
- 관리자 크레딧
  ...
  - `src/features/settings/tabs/AdminCreditSettingsTab.tsx` (`variant=credits`: 가입 무료 테스트 안내·수동 지급 기본액·배송 / `variant=customAbut`: 고시 생산·디자인+생산·딜러없음 분배·레거시 기공소 오버레이·환봉 추가요청)
...
- 역할별 정산
  ...
  - 제조사: `src/pages/manufacturer/payments/PaymentsPage.tsx` — 거래 원장(일시·지급상태·금액·잔액·거래내역). 유형 열은 생략(모두 커스텀어벗 생산+배송비). **생산·배송은 KST 하루 1행**(의뢰 1건=어벗 1개라 기공의뢰처럼 못 묶음). 클릭 상세는 의뢰/배송을 별 섹션으로 나누고, 그 안에서 **배송자 BA**(requestor businessAnchor·우편함)별. PTX는 치과명이 아니라 기공소 BA로 표기. 장부는 공급가(어벗 1개당 매입가 기본 8,800 부가세 포함→공급가). 리메이크만 무료 생산(지급 0). 무료 크레딧 결제 포함 약정 단가가 미정산으로 쌓이며 말일 일괄 지급(지급 시 +VAT·세금계산서). 정산규칙 모달은 플랫폼 설정 매입가·배송단가를 표시.
...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    - 내역 상단 요약은 **수식 카드**(`현재 잔액 = 유료 충전 [+ 정산 적립] − 소비`). 「무료 충전」카드 없음. 연산 부호만 카드 사이, 설명은 툴팁.
    ...
    - **치과**: 유료크레딧만 표시(잔여 무료>0일 때만 보조). `기공크레딧` 잔액 행·정산 필터 숨김. **기간 합계**·드릴다운은 필터 줄 **오른쪽** `PeriodFilter`(기간 전용 state). 내역 테이블은 기간 무관. API `from`/`to`는 `appendPeriodQueryParams`(KST YMD SSOT). 데모 모드면 충전 카드「충전」(유료 아님)·잔고 힌트·뱃지「데모 N일 남음」.
    ...
    - **통계 탭**(`CreditStatisticsTab`): 치과는 `유료 − 소비 | 의뢰건수`. 기공소는 **치과로부터 수신**(정산 적립·치과별·보철)과 **어벗츠로 의뢰**(유료 − 소비·유형)를 구역 분리. 파트너 패널=`치과별 적립`(보류 포함, 정산만). 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
...
- 부가세(VAT) / 면세 UI 정책(강제, 루트 `rules.md` §2.3) — 겸영 이중 체계:
  ...
  - 제조사 정산규칙: 원청–하청 고정 매입가(의뢰·배송, 부가세 포함)·장부 공급가·지급 시 VAT·세금계산서. 리메이크만 무료 생산(0). 무료 크레딧 포함 약정 단가 전액 · 말일 일괄 지급. % 분배 안내 금지.
...
- 제조사 정산(일별) 표시 정책:
  - 하청 고정단가: **어벗 1개당** 매입가(기본 8,800 부가세 포함) / 배송 박스당 3,500(부가세 포함). 상단 카드·상세는 의뢰/배송을 **분리** 표시(공급가·건수). 리메이크만 지급 0. 무료 크레딧 포함 약정 단가 지급 시 +VAT·세금계산서.
  ...
  - 제조사 지급 대상은 **약정 하청비 전액**(공급가 + 부가세 · 세금계산서). 리메이크만 제외하고 무료 크레딧 포함 약정 단가를 지급한다. 매달 말일 일괄 지급 전까지 상단「미정산」에 쌓인다. 지급 완료분은「지급 합계」.
