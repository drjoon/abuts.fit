...
- 요약 카드 (`SettlementStatCard`, 정산·크레딧 상단 요약):
  ...
  - 내역: `현재 잔액 = 유료 충전 + 데모/무료 [+ 정산 적립] − 소비`. 통계: `유료 + 데모/무료 [+ 정산 적립] − 소비 | 의뢰건수`.
  - **데모/실사용 구분**: 내역·통계 공통 체크박스(`CreditUsageScopeFilter`, query `usageScope=real|demo|all`). 데모 내역이 있으면 첫 방문 시 깜빡임 안내(localStorage). 실사용만 보면 잔액 수식과 맞음(기공소 데모 적립 보류는 잔액 미반영).
...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    ...
    - **통계 탭**(`CreditStatisticsTab`): 상단 `유료 충전 + 데모/무료 충전 [+ 정산 적립] − 소비 | 의뢰건수`. 기공소 파트너 패널=`치과별 적립`(보류 포함), 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
...
- 헥스 회전 라벨은 **표시(UI)와 전달(canonical)을 동일 문자열**로 통일합니다.
  ...
  - UI/백엔드 canonical: `STL모델대로` / `헥스30도회전` / `STL모델+` / `헥스30+`
    ...
    - NC C축: T4848 항상 `C0.0`; T0909/T0606=`C(modeBase+addDeg)`, `addDeg=30+appliedDeg`
