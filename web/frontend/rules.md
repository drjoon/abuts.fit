...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 안내 모달([정책 안내])의 리메이크: **치과로부터(PTX) = 무료**, **어벗츠로(Request) = 건당 10,000원**. 동일 치과·환자·치식·최근 **180일** 조건 충족 시 적용(월 무료 쿼터 없음).
  - 관련 파일:
    - `src/shared/ui/PricingPolicyDialog.tsx`
    - `src/pages/requestor/dashboard/components/RequestorPolicyRemakeHeader.tsx`
...
- practice 전송 상태 표준(치과/의뢰자 공통): `발송완료 | 취소 | 수신완료 | 의뢰수락 | 자동매칭 | 작업완료 | 생산진행`
  ...
  - **PTX 리메이크**: 목록「리메이크」확인은 **커스텀어벗 기본 제외**(체크 옵트인). 포함 시 기공소→어벗츠는 동일 **180일** 조건에서 건당 10,000원. 치과→기공소 리메이크비는 원본 **180일** 이내면 무료(`LAB_FEE_REMAKE_FREE`). 목록 `remakeFeeQuote`=보철만, `remakeFeeQuoteWithCustomAbutment`=CA 포함.
  - **보철 종류 변경 리메이크**: 상세「보철 종류 변경」·다음 도착일 제안. 인레이→크라운 등. 기공비는 임시치아→지르와 같이 **단계 최고가만 청구**. 동일 종류는 일반 리메이크.
  - **신규 의뢰 동일 환자·치아**: 작성·전송 시 최근 **180일** overlap이면 리메이크/신규 확인(`PracticeSimilarCaseRemakeDialog`). 리메이크면 원의뢰 연결·기공소 「리메이크」뱃지.
