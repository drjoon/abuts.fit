...
- practice
  ...
  - `src/shared/components/practice/PracticeTransferRequestIntakePanel.tsx`
    ...
    - 기공소 의뢰카드·전송 상세: 수령액=`labSettlementAmount`(지정 기공소=기공비 전액, 자동매칭만 플랫폼 수수료 차감). 툴팁 컬럼 `기공소 몫`/`어벗츠 몫`(어벗츠 열은 금액 대신 `커스텀어벗`, 합계는 기공비만). 툴팁 트리거는「기공비 총액」텍스트만(우측 ? 제외). 치식 요약 `커스텀어벗`→`어벗츠 지급`. 목록 `feeQuote` SSOT.
