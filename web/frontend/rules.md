...
- practice
  ...
  - `src/shared/components/practice/PracticeTransferRequestIntakePanel.tsx`
    ...
    - 기공소 의뢰카드·전송 상세: 주 표기(합산 라벨)=`기공비`·금액=보철기공비+어벗 디자인+생산비. 하청이면 보조로 수령=`기공비×(1−subcontractFeeRate)`. 원청(어벗츠 기공사업부)이 하청을 준 뒤 자기 화면은 전액 수주(수수료 0). 지정 기공소는 전액(플랫폼 수수료 없음). 툴팁 트리거는「기공비」텍스트만(우측 ? 제외). 목록 `feeQuote` SSOT.
...
- 관리자 크레딧
  ...
  - `src/pages/devops/components/DevopsPlatformFeeTab.tsx` (하청 %. `PracticeTransferAutoMatchTab` 카드 안. SSOT `payoutRates.subcontractFeeRate`)
- 개발·운영사 설정
