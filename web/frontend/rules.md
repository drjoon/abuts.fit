...
- practice 전송 상태 표준(치과/의뢰자 공통): `발송완료 | 취소 | 수신완료 | 의뢰수락 | 자동매칭 | 작업완료`
  ...
  - 기공소 의뢰수락: 상세 다이얼로그 「전체 다운로드」+「수락」(자동매칭 클레임 중이면 `수락 [남은 시간 …]`)→ `POST .../mark-accepted`(과금). 파일 다운로드는 뱃지/과금과 무관. 작업완료/취소는 모달이 아니라 메인 카드에서.
...
- practice 최근 전송 기공소(기공소 선택 드롭다운) SSOT:
  ...
  - 신규 draft 생성(autosave·임시 저장): 환자명·메모·기공소·파일 중 하나 이상 필요. 치아 선택만으로는 목록에 올리지 않는다. 기존 draft 갱신 시에는 치아 변경도 동기화한다.
