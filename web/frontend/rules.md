...
- 신규 기공소 런칭 이벤트 가격 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    ...
    - **기공소 기공의뢰 장부**: 보철기공비와 디자인비+지그제작비는 같은 지급(기공소몫)이라 **한 기공의뢰 행**으로 묶는다. 금액 호버는 치과와 같이 디자인비를 기공비에 합산. 어벗츠몫이 남아 있으면 「일부 지급」. 행 클릭 상세는 기공소 견적 뷰. CA 디자인비 원장(`abutment_design_lab_fee`)을 생산의뢰 행으로 따로 표시하지 않는다.
