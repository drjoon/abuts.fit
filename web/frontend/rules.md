...
- 개발·운영사 설정
  ...
  - 의뢰자(치과) 설정: 구독 탭 없음. 구 `?tab=subscription` → 계정. 대시보드 헤더는 `[정책 안내]`만.
    ...
    - `src/shared/ui/PricingPolicyDialog.tsx` — 런칭 이벤트 1만(+배송) / 정상가 1.3만(+배송 또는 FM덴탈 월정액·기공소만). 신속 +2,000 · 배송 3,500. FM 가입·해지(대표·기공소). 기공소·딜러·영업팀 `requestor`: 플랫폼 사용료 관리자 %(초기 2%·학습 이용 허용 시 면제) · 하청 10%+사용료(허용 시 사용료만 면제). 본부는 항상 허용. **딜러(`variant=salesman`)**: 심플웨이·커스텀어벗 매출(기공 제외) 20% · 90일 무주문이면 소개 리셋. **「의뢰자 정책」(`variant=requestor`)**: 딜러 대시보드·영업팀 성과 — 소개 치과·기공소 안내용 단가·출고·기공소 수수료.
