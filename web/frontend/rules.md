...
- 요약 카드 (`SettlementStatCard`, 정산·크레딧 상단 요약):
  ...
  - 내역: `현재 잔액 = 충전 [+ 정산 적립] − 소비`(「무료 충전」카드 없음).
  - 통계(치과): `충전 − 소비 | 의뢰건수`.
...
- practice
  ...
  - `src/pages/practice/components/PracticeRecentTransfersAllModal.tsx` (최근 의뢰 전체보기: 기간 필터 + 2주/한 달 캘린더)
  - `src/pages/practice/components/PracticeRecentTransfersCalendar.tsx`
...
- 개발·운영사 설정
  ...
  - 의뢰자(기공소) 설정: `requestorKind=lab`일 때 알림 **왼쪽**에 「기공비」 탭
    ...
    - 구 `?tab=auto-match`·`trading-partners` → 계정. 인증 신청 UI 제거(관리자 `PracticeTransferAutoMatchTab`). 구 `?tab=ai` → 계정. 학습 이용 변경은 기공의뢰 「수수료 줄이기」
  ...
  - 의뢰자(치과) 설정: 구독 탭 없음. 구 `?tab=subscription` → 계정. 대시보드 헤더는 `[정책 안내]`만.
    ...
    - `src/shared/ui/PricingPolicyDialog.tsx` — 런칭 이벤트 1만(+배송) / 정상가 1.3만(+배송 또는 FM덴탈 월정액·기공소만). 신속 +2,000 · 배송 3,500. FM 가입·해지(대표·기공소). 치과·기공소·딜러·영업팀 `requestor`: 플랫폼 사용료는 매출액의 2%. 협력·하청 모두 차감 후 크레딧 적립. 협력은 영업 수수료 없음, 하청은 10% 추가. 학습 이용 동의 시 사용료 면제. 치과는 이 사용료 없음. **딜러(`variant=salesman`)**: 심플웨이·커스텀어벗 매출(기공 제외) 20% · 90일 무주문이면 소개 리셋. **「의뢰자 정책」(`variant=requestor`)**: 딜러 대시보드·영업팀 성과 — 소개 치과·기공소 안내용 단가·출고·기공소 수수료.
  ...
  - 기공의뢰 자동매칭: `PracticeTransferAutoMatchTab`(카드·탭 **인증 기공소**) — 수수료 스트립(하청%/지정 on·off·%) + 기공소별 인증 ON·기공 테스트·메모. 기공소 설정 탭은 없음. 관리자 테스트 통과/`enabled` 시 풀 참여. 지정·협력 플랫폼 사용료 **2%**(`directPlatformFeeRate`, 학습 이용 허용 시 면제). 하청 `subcontractFeeRate`(기본 10%)+사용료. 본부는 항상 허용. 학습 이용은 기본 허용. 기공소 설정 「AI」탭은 제거했고, 변경은 기공의뢰 「수수료 줄이기」. 기공의뢰 진입 확인은 오른쪽 위 X·바깥 클릭으로 넘길 수 있고, 첫 작업시작 확인은 사업자에 따로 저장한다. 예전 의뢰가 있어도 없으면 다음 작업시작에서 선택을 강제하고, 허용 안 함이면 사용료 청구를 한 번 더 확인한다. 관리자 플랫폼 설정「인증 기공소」탭
...
- 신규의뢰 첨부·구강스캔 묶음 (파일 크기 SSOT):
  ...
  - **3MB 초과 드롭/파일 오픈**: 첨부 전 `ConfirmDialog`로 확인. 3D 프리뷰(`StlPreviewViewer`) + «커스텀 어벗 STL만 받음» + 구강스캔이면 치과=`구강스캔으로`·기공소=`어벗츠로부터` 안내. 확인 시에만 어벗생산의뢰로 첨부. 치과 링크 클릭 시 `navigate(..., { state: { prefilledFiles } })`로 파일을 넘기고 `PracticeFileTransferPage`가 첨부(기공소는 수신 페이지로만 이동).
  - 자동 분류·묶음 (3MB 단일 기준):
...
- 신규의뢰 배송 방식(묶음/신속):
  ...
  - 디자인+생산(`design_custom_abutment`): `(생산 단가 + 디자인비) × 어벗 수`.
    ...
    - 안내·청구 정가 SSOT (`creditSettings` + `src/shared/pricing/abutsAbutmentService.ts`): 정상가 **1.3만원** · 런칭 **1만원**(`resolveCustomAbutmentProductionPriceForAt`). 신속 `expressFee`(기본 +2,000). 배송 박스당 또는(기공소) FM덴탈 월정액. `regular*`는 관리자 딜러분배용. 기공소 매칭 월정 0·지정/협력 플랫폼 사용료 2%(학습 이용 허용 시 면제) · 하청 10%+사용료 · 본부는 항상 허용 — 루트 `rules.md` §2.3.
...
- Next Up 카드 드래그 → 다른 장비 Next Up으로 이동 (`POST /api/cnc-machines/queues/move`). 이동 시 기존 NC를 삭제하고 CAM을 재생성한다. NC 미수신(재생성 중)이면 Next Up·예약 관리(`CncPlaylistDrawer`)에 옅은 틴트 + 반투명 「작업중/작업중지」 오버레이(준비 탭 라이노와 동일, 본문 블러 없음).
...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 기공소 어벗츠 인증: 관리자 `PracticeTransferAutoMatchTab`에서 신청·테스트·상태 관리. **월 참여 수수료 0원**(정책). 지정·협력 플랫폼 사용료는 **2%**(기공소 계정 학습 이용 허용 시 면제, 의뢰 생성 때 스냅샷). 하청 `subcontractFeeRate`(기본 10%)에 같은 사용료를 더한다. 어벗츠기공본부는 항상 허용. 구 거래 치과 소개 UI는 제거(초대 API는 레거시 유지). 구 기공소 설정「어벗츠 인증」탭 제거.
  - 치과향 지정 기공소 표시: `formatPracticeTargetLabLabel` → **「어벗츠 · {이름}」**(협력). 하청만 「어벗츠기공소 · 인증 협력 기공소에서 처리」. 라우팅 `targetLab`은 현행 유지.
  - 기공소 사이드 설정과 계정 팝업 사이: 가입 이유 배너. 어벗생산의뢰 상단은 생산 현황 헤더(`[정책 안내]`·진행중·출고예정·완료·불완전가공).
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    - 내역 상단 요약은 **수식 카드**(`현재 잔액 = 충전 [+ 정산 적립] − 소비`). 「무료 충전」카드 없음. 연산 부호만 카드 사이, 설명은 툴팁.
    - **요약 카드 클릭 → `CreditLedgerModal` 상세(필터 프리셋)**. 카드 위·사이 수식·설명 문단 상시 노출 금지. `.cursor/rules/ui-summary-cards.mdc`.
    - 테이블·필터·관리자 모달도 rounded-2xl·slate border 톤으로 통일.
    - **치과**: 유료크레딧만 표시(잔여 무료>0일 때만 보조). `기공크레딧` 잔액 행·정산 필터 숨김. **기간 합계**·드릴다운은 필터 줄 **오른쪽** `PeriodFilter`(기간 전용 state). 내역 테이블은 기간 무관. API `from`/`to`는 `appendPeriodQueryParams`(KST YMD SSOT). 충전 카드 라벨「충전」. **소비** 카드 하단 `완료/보류`(결제 HOLD). 데모 모드면 잔고 힌트·뱃지「데모 N일 남음」.
    - **기공소**: 치과와 동일 수식 카드 + **정산 적립**(카드 금액=확정 · 하단 `완료/보류`). 소비 카드에는 완료/보류를 붙이지 않음(적립 보류와 혼동 방지). 기간 필터는 카드만(테이블 기간 무관). 버킷 필터(유료/무료/기공)·동작(충전/소비/조정) 유지. `SPEND_SETTLEMENT`=기공크레딧 주문 사용.
    - **기공소 기공의뢰수신**: 의뢰카드·전송 상세에 수령액(청구 − 플랫폼 수수료) 표시.
    - **기공소 기공의뢰 장부**: 내역 행(지급상태·금액 호버·상세)은 **치과와 동일**. 보철기공비와 어벗 디자인+생산비는 모두 기공비. 금액 호버는 두 열(+배송비)·기공비 총액까지. 기공소→치과·치과→기공소 배송은 무료(기공수가 배송 항목 없음). 지급 상태는 **적립 보류/일부 적립/적립 완료**(치과는 결제 보류·완료). 커스텀어벗은 디자인 STL 업로드 후 어벗츠에 생산비가 지급될 때까지 기공소 **확정** 정산·지급에서 빠지며, 그 전에도 「적립 보류」와 치과 결제 보류·기간 소비는 유지한다(정책 안내·정산규칙·정산 적립 툴팁). lab-share HOLD는 기공크레딧 잔액·상단 정산 적립 합계에 넣지 않고 「적립 보류」행으로만 미러. 행 클릭 상세는 치과와 같은 견적 뷰(배송비·크레딧 소비 총액 포함). CA 디자인비 원장(`abutment_design_lab_fee`)을 생산의뢰 행으로 따로 표시하지 않는다.
    - **통계 탭**(`CreditStatisticsTab`): 치과는 `충전 − 소비 | 의뢰건수`(소비에 결제 보류 HOLD 포함). 기공소는 **어벗츠로부터**(정산 적립 확정·치과별·보철)과 **어벗츠로 의뢰**(충전 − 소비·유형)를 구역 분리. 파트너 패널=`치과별 적립`(확정만). 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
    - **치과 장부**: 구강스캔 기공의뢰=`기공의뢰-구강스캔으로`. 금액 호버는 견적 열 기준 **보철기공비**(지르 등 기공수가)·**어벗 디자인+생산비**(구강스캔 2.5만, 구강지그 제외)·**배송비**(장부 →어벗츠 박스 행만). 행 클릭 정산 상세는 기공비 총액 아래 배송비(기공소→어벗츠/치과→어벗츠=플랫폼 `shippingFee`, **의뢰 BA+출고일 1박스**)·크레딧 소비 총액. 기공소→치과·치과→기공소 배송 무료. **견적 툴팁·의뢰 상세 견적에는 배송비 없음**(묶음 발송). 정산 경로 보류(디자인비를 기공소몫에 합쳐 7만+1.5만으로 보이는 것)는 쓰지 않는다. 어벗디자인(치과)·어벗생산(기공소) 의뢰비·배송비는 **의뢰 사업자+예정 출고일** 1박스(`기공의뢰-어벗디자인으로`). 치과명으로 쪼개지 않음. 거래내역은 의뢰 사업자+건수, 클릭 시 의뢰/배송 상세(신속/묶음 뱃지). 지급 상태는 구강스캔과 같이 보류/일부 지급/지급 완료.
  - 안내 모달([정책 안내])의 리메이크: **치과로부터(PTX) = 기공소 `freeRemakeYears` 기간 이내 무료**(null/0=유료), **어벗츠로(Request) = 건당 10,000원**. 어벗츠 경로는 동일 치과·환자·치식·최근 **180일** 조건(월 무료 쿼터 없음).
  - 기공소 설정 「기공비」·관리자 「기공소 수가」: `labFeeSchedule.freeRemakeYears`(년). 초기 null → `LabFeeSetupPrompt`/`from=freeRemake` 하이라이트.
  - 관련 파일:
    - `src/shared/ui/PricingPolicyDialog.tsx`
    - `src/pages/requestor/dashboard/components/RequestorPolicyRemakeHeader.tsx`
...
- 스커리씁 로트 추적 UI(세척.패킹)는 `rules.legacy-full.md` 섹션 **1.0.3**을 따릅니다.
  ...
  - **각인 이미지 매칭**: 업로드 시 AI 인식은 제안만(즉시 포장.발송 금지). **인식 성공 건은 다음 촬영이 오면 자동 확정**. 틀리면 3글자 수정·카드 드롭으로 정정. BE `packing:capture-needs-confirm` / `requestMongoId` 확정.
...
- 제조사 워크시트 의뢰 정보 표시 SSOT (`RequestInfoSummary`):
  ...
  - PLY/OBJ/DCM 프리뷰 칼라: 버텍스 컬러 또는 `TextureFile`/동반 JPG·PNG(`createModelPreviewMaterial` + `parseModelPreview`). **MeshStandard(릿)** 로 칼라+음영을 같이 써서 외형선 유지(언릿 Basic 금지). 소프트 핑크 잇몸·진주 화이트 치아 블렌드(밝기↑) + `LinearToneMapping`(exposure 1.18, ACES 금지) + 배경 `SCAN_COLOR_PREVIEW_BACKGROUND`(흰색). 뷰어 좌상단 **칼라 매핑** 스위치로 ON/OFF. SSOT `src/shared/files/modelPreviewFile.ts` · `StlPreviewViewer`.
...
- 부가세(VAT) / 면세 UI 정책(강제, 루트 `rules.md` §2.3) — 겸영 이중 체계:
  ...
  - **스토어 기성품(과세)**: 치과·기공소 의뢰자. 고객 표시는 **부가세 포함가** + `과세 · 부가세 포함`. **BA.`storePackageBuyer`**이면 판매가 취소선 + pkg가(500만 패키지 `full-package` 결제 확정 시 자동 ON). `RequestorStorePage` / cart / orders. 헬퍼: `shared/tax/invoiceLabels.ts` · `shared/store/storeCatalog.ts`.
...
- 헥스 회전 라벨은 **표시(UI)와 전달(canonical)을 동일 문자열**로 통일합니다.
  ...
  - UI/백엔드 canonical: `STL모델대로` / `헥스30도회전` / `STL모델+` / `헥스30+`
    ...
    - NC C축: T4848 항상 `C0.0`; T0909/T0606=`C(modeBase+addDeg)`, `addDeg=30+appliedDeg` (appliedDeg 부호 포함, 예: -27.61 → C2.39)
...
- 제조사 정산(일별) 표시 정책:
  - 하청 고정단가: **어벗 1개당** 매입가(기본 8,800 부가세 포함) / 리메이크 6,600 / 배송 박스당 3,500(부가세 포함). 상단 카드·상세는 의뢰/배송을 **분리** 표시(포함가·건수; 푸터에 공급가 참고 가능). 무료 크레딧 포함 약정 단가 전액·지급 재가산 없음·세금계산서.
...
- 의뢰자 역할·서비스·유료게이트 SSOT (2026-08, 루트 §2.4 상세)
  ...
  - 온보딩(`/dashboard/wizard`): 프로필 → 휴대전화 → 역할 → 사업자.
    ...
    - 어벗츠 출시 행사 신청 폼은 구강 스캔 사용 여부를 항상 받는다. 프로필·사업자에 값이 있으면 그 값을 기본 선택한다.
...
- practice 전송 상태 표준(치과/의뢰자 공통): `발송완료 | 취소 | 수신완료 | 의뢰수락 | 자동매칭 | 작업완료 | 생산진행`
  ...
  - 기공소 작업시작: 상세 다이얼로그 왼쪽 「열기」(설정 디자인 SW·로컬 CAD 연결. 미설치 시 웹 모달에서 zip 받기→「여기를_더블클릭_설치」1회·이후 시작프로그램/`abuts-cad://` 자동)·「전체 다운로드」, 오른쪽 「치과와의 소통」 상단에 안내 문구+「작업시작」→ `POST .../mark-accepted`(과금·레거시 경로명). 파일 다운로드·열기는 뱃지/과금과 무관. 작업시작 후 같은 자리 2단: 안내 문구 / `[어벗 업로드 & 생산의뢰]`·`[보철 업로드 & 작업완료]`·`[작업취소]`. 작업시작 시 `practice:transfer-updated`(action=`accepted`, `feeQuote` 확정)로 치과 UI가「확정 기공비」를 즉시 표시.
  - **자동매칭(레거시)**: 치과 기공소 픽커의 「자동 매칭」항목은 제거. **신규 의뢰 계약 상대=어벗츠기공소(고정).** 픽커의 외부 기공소=협력(`assigneeKind=cooperation`, 0%). 어벗츠 지정 후 풀/클레임=하청(`assigneeKind=subcontract`, 10%). 기존 `matchingMode=auto` 건·공개 풀·우선창 엔진은 유지하되, 작성 UI에서는 쓰지 않는다. 레거시 draft는 어벗츠기공소로 복원. 표시명 마스킹·수신 뱃지 합산 규칙은 하청·레거시 auto에 적용. UI: `PracticeTransferAutoMatchTab` (관리자 플랫폼 설정「인증 기공소」)
  ...
  - **PTX 리메이크**: 목록「리메이크」확인은 **커스텀어벗 기본 제외**(체크 옵트인). 포함 시 기공소→어벗츠는 동일 **180일** 조건에서 건당 10,000원. 치과→기공소 리메이크비는 기공소 `freeRemakeYears` 이내면 무료(`LAB_FEE_REMAKE_FREE`). 목록 `remakeFeeQuote`=보철만, `remakeFeeQuoteWithCustomAbutment`=CA 포함.
  - **주문 변경**(구 「보철 종류 변경」리메이크): 상세「주문 변경」·다음 도착일 제안. 형태(인레이→크라운)뿐 아니라 **어벗(간접↔직접)·쉐이드·임플란트** 등 스펙 변경도 허용. 기공비는 임시치아→지르와 같이 **단계 최고가만 청구**. 완전 동일 주문은 거부(일반 리메이크).
  - **신규 의뢰 동일 환자·치아**: 작성·전송 시 최근 overlap이면 리메이크/신규 확인(`PracticeSimilarCaseRemakeDialog`). 감지 창은 무료기간 상한(년). 리메이크면 원의뢰 연결·기공소 「리메이크」뱃지. 무료 여부는 해당 기공소 `freeRemakeYears`.
...
- practice 최근 전송 기공소(기공소 선택 드롭다운) SSOT:
  ...
  - 기공의뢰 상단 툴바: 「새로 작성」·「최근 의뢰」·「임시저장」·「휴지통」. 최근 의뢰=전체보기 모달(`PracticeRecentTransfersAllModal`: 헤더에 상태 6뱃지+검색, 3주 세로 스크롤 캘린더, 년/월 오른쪽 요일 숨김, 칸 날짜=월/일, 기공소 정렬·색). 모바일 액션(신규·리메이크·임시·휴지·채팅)은 상단 크롬 유지 + 채팅형 전체화면 시트(`mobileActionOverlay.tsx`); 액션 전환 시 해당 시트만 메인, 닫으면 캘린더(채팅 복귀 아님). 기공소도 설정·어벗 진행중 동일. PC 임시저장/휴지통=다이얼로그. 우측 목록 없이 작성 폼 전폭(치식 full 차트, 카드 min-w로 어벗 라벨 표시). 본문 순서: 기공소·환자·일정 → **메모|드롭존 2열** → 보철물.
  - 신규 draft 생성(autosave) 및 갱신: 완성형 한글 환자명(가-힣)만 필요(기공소는 전송 시). 자모만(ㅎㅈ)·IME 중간 Latin은 저장하지 않는다. 치아·메모·파일만으로는 목록에 올리지 않는다. 환자명 입력 뒤의 기존 draft 갱신에서는 치아 변경도 동기화한다. 상단 툴바 「동기화됨」자리는 미충족 시 그 이유를 표시한다.
...
- 제조사 정산(`src/pages/manufacturer/payments/PaymentsPage.tsx`) 표시 정책:
  - 원장 `GET /api/manufacturer/credits/ledger`는 생산·배송 EARN을 **KST 하루 1행**으로 묶어 반환한다. 목록에서 유형은 생략. 상세(`mailboxGroups`)는 의뢰/배송 섹션으로 나누고, 그 안에서 **배송자 BA**(requestor businessAnchor·우편함)별 생산·발송·배송비. 지급은 별행. **조정(ADJUST)은 KST 하루 1행**으로 묶고, 클릭 상세는 의뢰별(같은 의뢰의 중복 환불은 합산, 사유 표시).
