...
- 요약 카드 (`SettlementStatCard`, 정산·크레딧 상단 요약):
  ...
  - 내역: `현재 잔액 = 유료 충전 + 데모/무료 [+ 정산 적립] − 기공, 스토어`. 통계: `유료 + 데모/무료 [+ 정산 적립] − 기공, 스토어 | 의뢰건수`.
...
- 커스텀 어벗 의뢰 단가 표시 SSOT:
  ...
  - 크레딧 잔액·장부 UI(`CreditLedgerModal` / 의뢰자 크레딧 페이지):
    - 내역 상단 요약은 **수식 카드**(`현재 잔액 = 유료 충전 + 데모/무료 충전 [+ 정산 적립] − 기공, 스토어`). 연산 부호만 카드 사이, 설명은 툴팁.
    ...
    - **통계 탭**(`CreditStatisticsTab`): 상단 `유료 충전 + 데모/무료 충전 [+ 정산 적립] − 기공, 스토어 | 의뢰건수`. 기공소 파트너 패널=`치과별 적립`(보류 포함), 치과=`기공소별 소비`. 차트·리스트 클릭 → 내역 모달.
