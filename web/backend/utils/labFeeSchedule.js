...
// - 2026-09-15: 심플힐링=스캔바디 일종 → 커스텀어벗 수가 과금(심플어벗/심플밀링만 제외).
// - 2026-08-25: 심플어벗(치과 재고)은 기공소 어벗 수가·견적에서 제외. 스캔바디(·심플힐링) 커스텀어벗만 과금.
...
/** 심플어벗/심플밀링 — 치과 재고. 기공소 커스텀어벗 수가·견적 제외. 심플힐링은 스캔바디 일종이라 과금. */
const SIMPLE_ABUTMENT_KINDS = new Set(["심플어벗", "심플밀링"]);

/**
 * 크라운·브리지·임시치아 + 심플어벗/심플밀링만 기공소 CA 수가에서 제외.
 * 심플힐링·스캔바디는 커스텀어벗을 만드므로 과금.
...
/** FDI 상악 16치 (18→11→21→28). 견적 라인 전체 치열 축약용. */
...
/** 상·하악 전체는 상악/하악. FE formatToothNumbersForFeeLine 와 동일. */
function formatToothNumbersForFeeLine(teeth) {
  ...
  if (
    UPPER_ARCH_TEETH.length > 0 &&
    UPPER_ARCH_TEETH.every((tooth) => selected.has(tooth))
  ) {
    parts.push("상악");
  } else {
  ...
  if (
    LOWER_ARCH_TEETH.length > 0 &&
    LOWER_ARCH_TEETH.every((tooth) => selected.has(tooth))
  ) {
    parts.push("하악");
  } else {
...
function followUpFinalProsthesisTeeth(row) {
