...
- 리퍼럴 코드 랜덤, 추천인: **고정 배정** (간접 수수료 검증 가능하도록)
  - **영업자 소개 40% (8명)**: r001~r008 → s001/s004/s006/s009가 균등 분배 (직접 수수료 5%, 상위 영업자는 간접 50% 룰 검증)
  - **의뢰자 소개 40% (8명)**: r009~r016 → r001~r004가 추천인(조직 공동 오너)으로 지정, requestor-to-requestor 리퍼럴 체인 검증
  - **미소개 20% (4명)**: r017~r020 → `referredByUserId = null` (독립 계정)
- 입금: 50만/100만/200만/300만원 4종 중 랜덤으로 `CreditLedger` CHARGE 생성
- 보너스: 조직당 30,000원 `CreditLedger` BONUS 생성 (무료 크레딧, 의뢰에서만 사용)
- 의뢰 기록: **최근 20일 내 20~50건 랜덤 생성**, 금액 정책:
...
- 배송비: 완료 의뢰를 3~20개씩 묶어 `ShippingPackage` 생성, 패키지당 3,500원 SPEND 생성 (배송비는 **유료 크레딧에서만 차감**, 무료 크레딧 사용 불가)
