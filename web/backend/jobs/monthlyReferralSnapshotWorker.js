/**
 * 매달 1일 KST 00:00에 실행되는 리퍼럴 그룹 스냅샷 재계산 워커.
 *
 * 지난달(전월 1일~말일) 완료 의뢰 기준으로 각 그룹 리더의
 * groupTotalOrders / groupMemberCount 를 PricingReferralStatsSnapshot에 upsert한다.
 * 이 스냅샷이 당월 의뢰 단가 계산의 기준이 된다.
 *
 * 실행 방식: 별도 프로세스로 기동 후 매 1분마다 KST 기준 1일 00:00 여부를 확인하여 실행.
 * (node-cron 미사용 - 기존 워커 패턴과 동일)
 */
