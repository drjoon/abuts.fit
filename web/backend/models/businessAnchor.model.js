...
const businessAnchorSchema = new mongoose.Schema(
  {
    ...
    fmDentalShippingCancelAtPeriodEnd: {
      type: Boolean,
      default: false,
    },
    ...
    fmDentalShippingStartedAt: {
      type: Date,
      default: null,
    },
    fmDentalShippingCanceledAt: {
      type: Date,
      default: null,
    },
    ...
    payoutRates: {
      ...
      // 기공의뢰 수수료율.
      // - subcontractFeeRate: 어벗츠 원청을 타 기공소가 하청 수행할 때 (기본 5%)
      // - platformFeeRate: 레거시 매칭 성공 수수료 (경로 B 자체 수행은 0)
      // - directPlatformFeeEnabled: 지정(direct) 수수료 적용 on/off(기본 off=없음)
      // - directPlatformFeeRate: 지정 요율 (기본 0)
      platformFeeRate: { type: Number, default: 0.1, min: 0, max: 1 },
      subcontractFeeRate: { type: Number, default: 0.05, min: 0, max: 1 },
      directPlatformFeeEnabled: { type: Boolean, default: false },
      directPlatformFeeRate: { type: Number, default: 0, min: 0, max: 1 },
      // 기공소 자동 매칭 월 참여 수수료(원). 정책상 0 고정. 관리자「기공소 매칭」.
    ...
    practiceTransferSettings: {
      ...
      // 전체치열 모달 좌측 보철물 목록(순서 유지). 기본=전체틀니·부분틀니.
      ...
      // 레거시: 기공의뢰 「디자인 컨펌 생략」계정 설정. 신규 의뢰는 항상 생략(true) 고정.
      skipDesignConfirm: {
        type: Boolean,
        default: true,
      },
