...
- 신속 배송(`express`) 복원 메모:
  ...
  - 디자인+생산 과금: `caseInfos.productMode === "design_custom_abutment"`일 때만
    ...
    - 단가: 치과 청구 SSOT=`creditSettings.membershipProductionPrice`(정상가 기본 13,000) · 런칭 이벤트 10,000(`customAbutmentLaunchEvent*`, `resolveCustomAbutmentProductionPriceForAt`). `membershipDesignAndProductionPrice`(기본 25,000, 레거시). `designFee`는 디자인+생산 − 생산만과 동기화(기본 10,000, **1어벗당**). 배송: 이벤트=박스당 · 정상가=박스당 또는(기공소만) FM덴탈 월정액(`fmDentalMonthlyShippingFee` · `FM_DENTAL_SHIPPING_SPEND`). 신속=`expressFee`(기본 +2,000). 치과 멤버십 월정 없음. 기공소 자동 매칭 **월 참여 수수료 0원**. 지정·협력 플랫폼 사용료 **0%**. 하청 `subcontractFeeRate`(기본 10%)만. 학습 이용 동의는 요율과 무관. 루트 `rules.md` §2.3.
...
- 커스텀 어벗 의뢰 단가 SSOT:
  - 관리자「플랫폼 설정 · 커스텀어벗」. 정상가=`membershipProductionPrice`(기본 13,000) · 런칭 이벤트=`customAbutmentLaunchEventProductionPrice`(기본 10,000) + on/off(**내일 0시 KST 예약**, 분배 비율과 동일). FM덴탈 월정액=`fmDentalMonthlyShippingFee`(기공소만·0이면 가입 불가). 신속은 +`expressFee`. `regular*`·「멤버/일반」은 딜러 유무 분배용.
...
- 기공소 기존 거래처 · 기공정산크레딧 SSOT:
  ...
  - **PTX 협력/하청 수가·할증 vs 정산(강제):**
    ...
    - **정산(공통):** 계약·선수금은 어벗츠→치과. 수행 기공소에는 어벗츠 gross 후 매입 이체(`resolvePracticeTransferSettlementParties`).
    - 헬퍼: `resolveFeeScheduleLabAnchorId` / `resolveLabFeeMultiplierLabAnchorId`. `isPracticeTransferSubcontracted`만으로 협력 수가를 어벗츠에 두지 말 것. Cursor `.cursor/rules/ptx-cooperation-fee-ssot.mdc` · 루트 `rules.md` §2.
  - PracticeTransfer 에스크로 SSOT: **생성(`POST /api/practice/transfers`)** 시 잔액 검사(`assertPracticeTransferPaidCreditSufficient`) + `holdPracticeTransferCredits`로 기공비 보류(`billing.heldAt`, `PRACTICE_TRANSFER_SPEND_HOLD`) — 수락 전 정산 페이지에 치과「결제 보류」/기공소「적립 보류」. **수락(`mark-accepted`)** 시 `adjustPracticeTransferHold`(보류액 확정) 직후 `releasePracticeTransferLabShare`로 정산(어벗츠 원청 gross→하청 매입; 레거시 외부 직접 지정만 수행 기공소 전액). **커스텀어벗은 치아별 디자인 STL이 있고 어벗 생산비가 어벗츠에 지급된 뒤에만** 이 확정 정산과 기공소 payout에 넣는다. 그 전에도 치과「결제 보류」·기공소「적립 보류」·기간 소비(보류)는 노출하고, 확정 적립·payout만 빼며, 조건 충족 시 그 시점 저널로 정산한다. UI 견적: `GET /api/practice/transfers/quote-context`(지정 기공소 스케줄·수수료율·어벗 단가) + 치식별 합산(`labFeeSchedule.js`). **신규 의뢰는 지정 기공소(어벗츠기공소 포함)만** — **수가·할증 앵커:** 협력(`assigneeKind=cooperation`)=수행 기공소(`labFeeSchedule`×`labFeeMultiplier`), 하청·어벗츠 자체=원청(어벗츠). 생성 시점 스냅샷·소급 금지. `resolveFeeScheduleLabAnchorId` / `resolveLabFeeMultiplierLabAnchorId`. 정산만 어벗츠 gross→수행 기공소 매입(크레딧 경유). 플랫폼 평균수가·별점 기공비 배수·평균수가 워커는 쓰지 않음. **치과 별점 하한·상한**(기본 3~4)은 지정 픽커·생성·하청 풀 수신 게이트(어벗츠 포함 유효 별점이 구간 안인 기공소만). 치과 평가 별점은 **수행 기공소**(하청 `assignee` 우선)에 남기며, 하청 시 치과에는 「인증 협력 기공소에서 처리」만 표시(실명 비공개·확정 후에도 동일). **우리 치과 1점**은 검색 가능·지정/하청 주문 불가. **어벗츠기공소 지정 의뢰**는 기공팀이 원하면 `open-subcontract`로 구간 안 인증 기공소 하청 풀을 연다. 치과 표시·과금은 원청(어벗츠) 수가+할증 유지. **수락(`mark-accepted`)** 시 보류 조정(`PRACTICE_TRANSFER_HOLD_ADJUST`)·`billing.billedAt` 확정. **기공소 발송=`mark-complete`** → 이미 수락 시 정산됐으면 release no-op(레거시 미정산만 lab share 해제), **제조사 발송=packing** → abutment share 해제. 목록 `feeQuote`: 치과=크레딧 소비(`total`), 기공소=설정 수가(`labFeeTotal`).
  - **PTX 리메이크(`POST /api/practice/transfers/remake`)**: 기본은 **커스텀어벗 제외**(보철만, `stripCustomAbutmentFromToothWorks`). `includeCustomAbutment=true`일 때만 CA 유지. **치과→기공소 리메이크비**: 기공소 `labFeeSchedule.freeRemakeYears`(null=미설정·유료, 0=유료, 1+=N년 이내 무료). 무료 창 안이면 `LAB_FEE_REMAKE_FREE` / `LAB_FEE_CUSTOM_ABUTMENT_REMAKE_DEFAULT_PRICE=0`. 창 밖·미설정이면 정가(비-리메이크) 견적. 이후 핸드오프 CA Request는 `computePriceForRequest` — 동일 치과·환자·치식·**180일**이면 **건당 10,000원**(`remake_fixed_10000`); CA STL 재업로드는 `forceRemakePricing`. 월 무료 쿼터 없음. `remake.includeCustomAbutment` 스냅샷. 목록 `remakeFeeQuote`=보철만, `remakeFeeQuoteWithCustomAbutment`=CA 포함. **동일 PTX `remakeCharges`**: 유료 리메이크 수가 정책 폐지로 무료 시 skip(`remake_fee_free`). 부위(`index`+보철|CA) **중복 hold 금지**.
  ...
  - **신규 작성 동일건 감지(`GET /api/practice/transfers/check-similar`)**: 환자명+치아(overlap)·감지 창 기본 **30년**(=`FREE_REMAKE_YEARS_MAX`). lean·견적 없음. 매치에 `freeRemakeYears`·`withinRemakePricingWindow`(기공소 설정 기준). 리메이크면 `POST /`에 `isRemake`+`remakeSourceTransferMongoId`로 원의뢰 연결·기공소 「리메이크」표시.
    ...
    - **지정·협력**: 플랫폼 사용료 **0%**. **하청**은 `subcontractFeeRate`(기본 10%)만. 학습 이용 동의는 요율에 반영하지 않는다. 이미 `billing.feeRateApplied`에 박힌 건은 소급하지 않는다. **어벗츠기공본부는 항상 학습에 포함.**
    - 걷힌 수수료 금액의 잔여 분배: 제조사는 하청 고정단가 경로와 분리. 수수료 잔액은 딜러사·개발운영사·어벗츠 상대비율로 재분배(루트 `rules.md` §2.3).
    - 자동 매칭 식별 정보: 레거시 `matchingMode=auto` 건만 마스킹 유지. **신규 의뢰 수가·할증:** 협력=수행 기공소, 하청·어벗츠 자체=원청. 정산만 어벗츠 경유. 치과 평가=별점만(기공비 할인/할증 없음, 수행 기공소·하청 포함). 하한·상한 설정으로 지정·하청 수신 게이트. 기공소「치과 평가」=할증. 자동매칭 신규 작성·별점 기공비 배수는 쓰지 않음.
  ...
  - eventType: `PRACTICE_TRANSFER_SPEND_HOLD` / `PRACTICE_TRANSFER_HOLD_ADJUST` / `PRACTICE_TRANSFER_ESCROW_RELEASE`(레거시 `PRACTICE_TRANSFER_SPEND_COMMIT` 유지); accountCode: `PLATFORM_ESCROW`, `LAB_SETTLEMENT_CREDIT`; creditKind: `SETTLEMENT`. 치과 장부: `기공비 보류` / 기공소: `기공크레딧` 적립(완료 시). **기공소 내역**: 작업완료 전 lab-share HOLD는 잔액에 넣지 않고 「적립 보류」행으로 미러(`listPendingLabSettlementLedgerRows`). **기간 요약**(`periodSpendSummary` / `ledger/stats`): 충전·소비(HOLD 포함) + (기공소) 정산 적립(**확정만**, 적립 보류 제외 — 잔액·수식과 일치). UI 요약 카드는 「무료 충전」제외(충전 [+정산] − 소비). 기공소 통계는 `settlementOrderCount`(치과→기공)·`abutsOrderCount`(기공→어벗츠)로 의뢰건수 분리, 파트너·보철유형은 정산 적립(확정)만. 데모/실사용 `usageScope`·`byUsage` 집계는 제거(단일 장부). UI 수식 카드 SSOT는 frontend `CreditLedgerModal`·`CreditStatisticsTab`.
  - 월 정산: 기공소 `SETTLEMENT_PAYOUT`으로 기공정산크레딧 → 계좌 이체. **다음 달 초 사용 유보 50만원(`LAB_SETTLEMENT_PAYOUT_RESERVE_WON`)을 남기고 나머지 잔액만 지급**(잔액 ≤ 유보면 해당 월 지급 0). 크레딧 페이지 탭은 내역·충전만(기공크레딧 정산 탭 없음; 내역 필터로 기공 버킷 조회).
...
- 제조사 헥스 회전 모드(`manufacturerHexRotation`)는 백엔드에서 fallback 기본값으로 보정하지 않습니다.
  - 허용값: canonical `STL모델대로` / `헥스30도회전` / `STL모델+` / `헥스30+`
- 제조사 아노다이징 override는 `caseInfos.anodizingEnabled` SSOT로 저장합니다.
  ...
  - 플러스 모드 SSOT: `STL모델+` modeBase=0.0 / `헥스30+` modeBase=30.0
    - NC: T4848 항상 `C0.0`; T0909/T0606=`C(modeBase+addDeg)`, `addDeg=30+appliedDeg` (부호 포함, 예: appliedDeg=-27.61 → C2.39)
...
- practice 전송 상태 표준(치과/의뢰자 공통)은 `발송완료 | 취소 | 수신완료 | 의뢰수락 | 자동매칭 | 작업완료 | 생산진행`을 사용합니다.
  ...
  - 과금 SSOT: 작업시작=`adjustPracticeTransferHold` + `releasePracticeTransferLabShare`(보철만, 또는 CA인데 디자인 STL·생산비 지급이 이미 끝난 경우. `+lab_platform_fee`, 치과 차감·기공소 정산; 경로 `mark-accepted`). **CA는 디자인 STL 미업로드면 확정 정산·payout 제외(적립 보류는 노출).** 보철 업로드 요구가 켜진 스냅샷이면 생산비 미지급도 제외. 협력 기공소이고 요구가 꺼지면(`billing.requireLabProsthesisUpload===false`) 생산비 게이트는 건너뛰고 모델 STL이 찼을 때 기공 보류를 푼다. 어벗츠 내부·하청은 설정과 관계없이 완성 보철 업로드가 필수다. 제조사 몫은 포장·발송. 생산비 지급(기공소 부담=가공 진입 `REQUEST_SPEND_COMMIT`, 비거래처 선불=제조사 발송 `releasePracticeTransferAbutmentShare`) 시점에 치과→기공소 정산. 작업완료=`mark-complete`(파일 선택, 이미 정산면 release no-op). 제조사 발송=`releasePracticeTransferAbutmentShare`. 파일 다운로드는 상태·과금에 영향 없음
  - **비거래처 선불 어벗 해제는 제조사 발송 유지(가공 진입으로 옮기지 말 것).** 생성 시 어벗 소매가가 PTX 보류에 있어 `ensureRequestCreditSpendOnMachiningEnter`는 `practicePrepaid && !isTradingPartner`에서 생산 차감을 스킵한다(이중 청구). 수취는 `releasePracticeTransferAbutmentShare` 한 키(`practice_transfer:{id}:escrow_release_abutment`, 의뢰 전체)이고 호출은 포장.발송만. 가공 진입으로 앞당기면 치아 하나만 들어가도 생산비 전액·제조사 매출(`REV_MANUFACTURER`)이 풀리고, 발송 전 취소는 보류 해제로 끝나야 하는데 매출 롤백과 잔여 치아 재보류가 필요하다(부분 취소 경로 없음). 제조사 정산도 발송보다 빨라진다. **배송비는 이유가 아니다.** `SHIPPING_SPEND_COMMIT`은 포장 때 박스 단위로 따로 확정되고, 기공소→치과 배송은 무료다. 루트 `rules.md` §2.2.
...
- LedgerJournal 스키마 초안(필수 필드):
  ...
  - `eventType`(저장형):
    ...
    - `CHARGE_PAID`, `CHARGE_FREE_REQUEST`, `CHARGE_FREE_SHIPPING`, `LAB_SETTLEMENT_CHARGE`, `ADJUST`, `SETTLEMENT_PAYOUT`, `STORE_SALE`, `REFUND`
...
- 승인/롤백 이벤트 정책(강제):
  - `REQUEST_SPEND_HOLD` / `SHIPPING_SPEND_HOLD`: **의뢰 제출**(단독 어벗디자인·어벗생산) 또는 **PTX CA 디자인 핸드오프(`design-handoff`)** 시 `PLATFORM_ESCROW` 보류. 치과 어벗디자인·기공소 어벗생산·PTX CA 생산·배송비는 의뢰 사업자+예정 출고일 1회(치과명 무관). PTX 수락(`mark-accepted`)에서는 CA Request만 조기 생성하고 hold는 잡지 않음. PTX 건당 abuts hold 폐지.
    같은 제출/핸드오프에서 원장 잔액 집계·CreditBalanceGuard 락은 앵커당 1회 재사용(`holdRequestCreditsOnSubmit`).
  ...
  - 준비 단계 **취소**: 미전환 REQUEST/SHIPPING HOLD 전부 물리 삭제(잔액 복원). 제조사 경로만.
...
- 부가세(VAT) / 면세 정책(강제, 루트 `rules.md` §2.3) — 겸영(과세+면세) 이중 체계:
  ...
  - 스토어 단가: 판매가 기본. **BA.`storePackageBuyer`**(500만 패키지 `full-package` 결제 확정 시 자동 ON·관리자 수동)이면 pkg가. 장부 누적 재조회 없음. `storeCatalog.js` · `storePackagePricing.js`.
...
- 정산/지급 정책:
  ...
  - 수익 분배 계산 SSOT는 `services/creditRevenuePolicy.service.js`를 사용합니다.
    ...
    - **딜러십 파트너 조건**: 영업 수수료=심플웨이·커스텀어벗(기공·배송 제외). **요율 20% 고정**. **유치 시점 요율 고정**(BA `dealershipCommissionRate`). **90일 무주문이면 소개 리셋**. 재유치 시 20% 스탬프. `resolveDealershipRateForAcquiredAt` · 관리자「플랫폼 설정 · 분배 비율」딜러십. (루트 `rules.md` §2.3).
    - 기공의뢰 수수료: 지정·협력 플랫폼 사용료 **0%**. **하청** `subcontractFeeRate`(기본 **10%**)만. 학습 이용 동의는 요율과 무관. 어벗츠기공본부는 항상 학습에 포함. · 월 참여 `autoMatchMonthlyFee`(**정책 0원**) — 관리자 플랫폼 설정「인증 기공소」. 루트 `rules.md` §2.3.
...
- 우편함/배송 무결성 정책(포장.발송):
  - 우편함 배정 SSOT (가공→세척.패킹 진입):
    ...
    - 배송비 보류 SSOT: 의뢰 제출(및 PTX CA `design-handoff` 시 미러 Request)에서 의뢰 사업자+예정 출고일 1회 `SHIPPING_SPEND_HOLD`(치과명으로 쪼개지 않음). 우편함 합류 시 중복 보류 해제. **포장.발송 진입** 시 `enterManufacturerShippingStage()` → 보류→매출(`SHIPPING_SPEND_COMMIT`). **기공소→치과·치과→기공소 배송 무료**(PTX `lab_shipping` hold 미생성·레거시는 해제).
