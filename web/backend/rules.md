...
- 단일 SSOT 장부(General Ledger) 정책:
  ...
  - 제조사 원장 **표시**는 의뢰 1건=어벗 1개라 기공의뢰처럼 못 묶고, KST 하루 단위로 생산·배송을 묶는다. 클릭 상세 데이터는 우편함(수취자)별이며, 프론트는 의뢰/배송을 별 섹션으로 나눈다. 구현: `utils/manufacturerLedgerDisplay.js`.
