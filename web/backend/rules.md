...
- 기공소 기존 거래처 · 기공정산크레딧 SSOT:
  ...
  - **PTX 리메이크(`POST /api/practice/transfers/remake`)**: 기본은 **커스텀어벗 제외**(보철만, `stripCustomAbutmentFromToothWorks`). `includeCustomAbutment=true`일 때만 CA 유지. CA 포함 시 치과→기공소 리메이크비에 CA 리메이크 합산 — 기공소 `labFeeSchedule` CA 리메이크 수가 미설정(0)이면 **개당 20,000원**(`LAB_FEE_CUSTOM_ABUTMENT_REMAKE_DEFAULT_PRICE`, 생성 차단 없음). 이후 핸드오프 CA Request는 `computePriceForRequest` **건당 10,000원**(`remake_fixed_10000`, 월 무료 쿼터 없음; 정가 `ptx_abuts_production` 금지). `remake.includeCustomAbutment` 스냅샷. 목록 `remakeFeeQuote`=보철만, `remakeFeeQuoteWithCustomAbutment`=CA 포함. **동일 PTX `remakeCharges`**: 기공소 수동(`lab_charge`)=**보철만**(유료 리메이크 수가); CA 치과→기공소 청구 SSOT는 **CA STL 재업로드(`ca_reupload`)**. 부위(`index`+보철|CA) **중복 hold 금지**.
