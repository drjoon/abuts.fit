...
- practice 전송 상태 표준(치과/의뢰자 공통)은 `발송완료 | 취소 | 수신완료 | 의뢰수락 | 자동매칭 | 작업완료 | 생산진행`을 사용합니다.
  ...
  - 잔액 검사 SSOT: `POST /api/practice/transfers`에서 `assertPracticeTransferPaidCreditSufficient`(유료+무료 합산). 생성 시 hold 없음.
