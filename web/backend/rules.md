...
- 기공소 기존 거래처 · 기공정산크레딧 SSOT:
  ...
  - **PTX 리메이크(`POST /api/practice/transfers/remake`)**: 기본은 **커스텀어벗 제외**(보철만, `stripCustomAbutmentFromToothWorks`). `includeCustomAbutment=true`일 때만 CA 유지. **치과→기공소 리메이크비**: 원본이 최근 90일 이내이면 무료(`LAB_FEE_REMAKE_FREE` / `LAB_FEE_CUSTOM_ABUTMENT_REMAKE_DEFAULT_PRICE=0`). 창 밖이면 정가(비-리메이크) 견적. 이후 핸드오프 CA Request는 `computePriceForRequest` — 동일 치과·환자·치식·90일이면 **건당 10,000원**(`remake_fixed_10000`); CA STL 재업로드는 `forceRemakePricing`. 월 무료 쿼터 없음. `remake.includeCustomAbutment` 스냅샷. 목록 `remakeFeeQuote`=보철만, `remakeFeeQuoteWithCustomAbutment`=CA 포함. **동일 PTX `remakeCharges`**: 유료 리메이크 수가 정책 폐지로 무료 시 skip(`remake_fee_free`). 부위(`index`+보철|CA) **중복 hold 금지**.
