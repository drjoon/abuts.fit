...
- 기공소 기존 거래처 · 기공정산크레딧 SSOT:
  ...
  - **신규 작성 동일건 감지(`GET /api/practice/transfers/check-similar`)**: 환자명+치아(overlap)·최근 **180일**(=`REMAKE_POLICY_WINDOW_DAYS`). lean·견적 없음. 리메이크면 `POST /`에 `isRemake`+`remakeSourceTransferMongoId`로 원의뢰 연결·기공소 「리메이크」표시. 수가 창도 동일 180일.
    ...
    - **지정 기공소**(`matchingMode=direct`): `payoutRates.directPlatformFeeEnabled`가 false가 아니면 `directPlatformFeeRate`(기본 **1%**) 적용. **기본 on**(관리자「인증 기공소」에서 off/요율 변경). 레거시 off+5% 저장분은 on·1%로 승격.
...
- 정산/지급 정책:
  ...
  - 수익 분배 계산 SSOT는 `services/creditRevenuePolicy.service.js`를 사용합니다.
    ...
    - 기공의뢰 성공 수수료: 매칭 `platformFeeRate`(기본 10%) · 지정 `directPlatformFeeEnabled`(기본 **on**) / `directPlatformFeeRate`(기본 **1%**) · 월 참여 `autoMatchMonthlyFee`(**정책 0원**) — 관리자 플랫폼 설정「인증 기공소」. 루트 `rules.md` §2.3.
