...
- practice 전송 상태 표준(치과/의뢰자 공통)은 `발송완료 | 취소 | 수신완료 | 의뢰수락 | 자동매칭 | 작업완료 | 생산진행`을 사용합니다.
  ...
  - 수락 전(의뢰=`발송완료|수신완료|자동매칭`) 내용 수정: `POST /api/practice/transfers/:transferId/update-content`. transferId·채팅방 유지, 파일·치식·메모·기공소·보류액 갱신, `requestorReadAt` 초기화, `practice:transfer-updated` action=`content-updated`. 수락·취소·거부 이후 409.
