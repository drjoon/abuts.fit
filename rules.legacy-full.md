...
- 영업자 소개 없이 가입한 의뢰자의 주문건: **설정된 영업자 분배비의 절반을 제조사, 나머지 절반을 관리자**에 가산(영업자 0%). 기본값이면 생산자 65% / 관리자 25% / 개발·운영사 10%
...
- 영업자 소개 관계(`BusinessAnchor.referredByAnchorId`의 대상이 `salesman`이 아닌 경우 포함)가 없는 의뢰는 영업자 몫을 배제하고, **설정된 `salesmanRate`의 절반을 제조사·관리자에 각각 가산**합니다(`resolveRatesWithoutSalesman`). 기본값(영업자 10%)일 때만 위 표의 65%/25%가 됩니다.
