...
- 우편함 배정은 가공→세척.패킹 진입에서 `mailbox.utils.assignMailboxForCleaningPackingEnter`를 단일 SSOT로 사용합니다.
- 세척.패킹→포장.발송 전환에서는 이미 배정된 우편함을 유지합니다 (`retainMailboxOnShippingEnter`). 없을 때만 같은 SSOT로 1회 보정합니다.
...
- 빈칸 할당은 집하 전 추적관리 점유 칸을 사용하지 않습니다. 합류는 세척.패킹/포장.발송/가공(우편함 유지)을 대상으로 합니다.
- 포장.발송 → 세척.패킹 롤백에서도 우편함을 유지합니다. 세척.패킹 → 가공 롤백에서만 해제합니다.
...
### 4.2.1 리메이크 과금 규칙 (2026-09-09)

- 리메이크 판정은 기존과 동일하게 **최근 90일 내 동일 치과명 + 환자명 + 치아번호** 기준을 사용합니다.
- 리메이크로 판정된 의뢰는 **건당 10,000원**을 적용합니다. (`price.rule = remake_fixed_10000`)
- ~~사업자(기공소) 단위 월 3건 무료~~ — 폐지(2026-09-09).
- 리메이크가 아닌 일반 의뢰는 기존 기본 단가 정책(플랫폼 설정 생산 단가)을 따릅니다.
- 배송비는 리메이크 여부와 관계없이 **별도 청구**합니다.
- 관련 파일: `web/backend/controllers/requests/utils.js`, `web/backend/controllers/requests/creation.from-draft.controller.js`, `web/backend/controllers/requests/dashboard.controller.js`, `web/frontend/src/shared/ui/PricingPolicyDialog.tsx`, `web/frontend/src/features/requests/components/RequestDetailDialog.tsx`, `web/frontend/src/pages/requestor/dashboard/components/RequestorRecentRequestsCard.tsx`
...
- 배송비의 canonical 규칙은 **집하(우편함 비우기) 시 차감 / 포장.발송 롤백 시 레거시 패키지만 환불**입니다.
...
- **주의사항**:
  - 실제 차감은 여전히 CAM 승인(의뢰비) / 집하(배송비) 시점에 발생
...
- **가공 완료로 세척.패킹 단계에 진입할 때** 자동 할당하며, 이미 같은 수신자가 보유한 우편함이 있으면 재사용합니다.
- 포장.발송 진입·포장.발송↔세척.패킹 롤백·세척.패킹→가공 롤백에서는 이미 배정된 우편함을 유지합니다. 가공→준비 롤백 시 `mailboxAddress = null`로 해제합니다.
- 배송비는 **집하(우편함 비우기) 시점**에 같은 박스(묶음 배송 패키지) 기준으로 **1회만** 차감합니다.
...
- 배송 추적/집하 동기화는 집하 완료 시 우편함 단위 배송비 과금 시점입니다.
