...
- **부가세(VAT) / 면세 정책(강제) — 겸영(과세+면세) 이중 체계:**
  - **어벗츠는 겸영사업자**다. 면세 매출(기공·커스텀어벗·크레딧)과 과세 매출(스토어 기성품·제조사/딜러/개발운영 지급)을 분리한다.
  - **면세(치과–기공소–어벗츠 · 커스텀어벗·크레딧)**: 크레딧 충전·소비·기공정산·치과↔기공소 기공의뢰·어벗츠 잔여·커스텀어벗 기공품(고객 청구). `vatAmount = 0`. 증빙은 **계산서**(세금계산서 아님). 이 경로의 고객/약관 가격 안내에 "VAT 별도·부가세 포함·VAT 10%" 금지(배송비 별도 안내는 유지).
  - **과세(스토어 기성품 · 어벗츠↔제조사 · 어벗츠↔딜러사 · 어벗츠↔개발운영사)**:
    - **스토어(치과 공급 기성품)**: 부가세 **10%**. 고객 표시는 **부가세 포함가**. 증빙은 **세금계산서**. 커스텀어벗 기공품과 동일 장바구니/동일 (세금)계산서에 합치지 않는다.
    - **제조사·딜러사·개발운영사 지급**: 장부·미정산은 **부가세 포함가**. 지급은 잔액 그대로(재가산 없음). 세금계산서는 포함가÷1.1 분해. 어벗츠가 수취하는 **세금계산서**. 제조사는 일반과세사업자(치과의사 대표·기공소 전환 없음).
  ...
  - 크레딧은 선불전자지급수단(선불페이)이 아니라 **B2B 거래 선수금/예치금**(플랫폼 내 계약 물품·용역: 기공·커스텀어벗·스토어 기성품만). 충전 화면·FAQ·약관·입금 확인에 `크레딧(거래 선수금)`을 명시한다.
  - 크레딧 충전: `vatAmount = 0`, `amountTotal = supplyAmount`. **입금(충전) 시점에는 (세금)계산서를 발행하지 않는다.**
  - **월말 고객 증빙**: KST 전월 사용분을 사업자당 **면세 계산서**(기공·어벗·배송 유료 소비)와 **과세 세금계산서**(스토어 `STORE_SALE`)로 **각각** 합산 발행(`ABUTS_TO_CUSTOMER`, SELF). 한 장에 면세·과세를 섞지 않는다.
...
- **어벗츠 사업 다각화 SSOT:**
  ...
  3. **기공사업부** — 어벗츠기공소(`internalLab`)가 치과와 **직접 계약**(원청). 선수금·결제는 항상 어벗츠 서비스로 1차 결제 후 수행 기공소로 이체. 외부 수행은 두 종류:
     - **협력**(`assigneeKind=cooperation`): 치과가 픽커에서 외부 기공소 직접 지정. **수가표·할증=수행 기공소**(치과↔지정과 동일). 정산만 어벗츠 gross→수행 매입. 플랫폼 사용료는 관리자 설정(초기 **2%**). 기공소 계정이 학습 이용을 허용하면 그 사용료는 면제(0%). 생성 때 스냅샷하며 이후 동의 변경은 소급하지 않는다. 치과 표시「어벗츠 · {파트너}」.
...
- **매칭 과금 SSOT(강제):**
  - 한 줄: **지정·협력 플랫폼 사용료는 관리자 설정(초기 2%). 학습 이용을 허용하면 면제. 하청은 `subcontractFeeRate`(기본 10%)에 같은 사용료를 더하고, 허용하면 그 사용료만 면제. 어벗츠기공본부는 항상 허용. 기공소 월 참여 0원.**
  - 기공소(`lab`): 자동 매칭 **월 참여 수수료(`autoMatchMonthlyFee`)는 0원 고정(정책)**. 지정·협력 **플랫폼 사용료**는 `directPlatformFeeRate`(초기 **2%**). 기공소 계정 `aiTrainingConsent`는 **기본 허용**. 설정 「AI」탭은 없고, 변경은 기공의뢰 「수수료 줄이기」. 아직 답을 안 한 기공소는 기공의뢰 진입 때 확인 모달로 묻는다. 오른쪽 위 X·바깥 클릭으로 넘길 수 있고, 첫 작업시작 확인은 사업자 `aiTrainingConsent.firstWorkStartConfirmedAt`에 따로 남긴다. 예전 의뢰가 있어도 이 값이 없으면 다음 작업시작에서 허용 여부를 묻고, 허용 안 함이면 사용료 청구를 한 번 더 확인한다. 허용이면 의뢰 생성(하청은 작업시작) 때 **0%로 스냅샷**. 나중에 켜거나 꺼도 이미 나간 건은 그대로. 거부한 건은 `aiTraining`에도 넣지 않는다. 마진·스캔 품질로 면제를 깎지 않는다. **어벗츠기공본부(`internalLab`)는 항상 동의한 것으로 보고 학습에 포함한다.** 하청은 `subcontractFeeRate`(기본 **10%**) + 플랫폼 사용료. 허용 스냅샷이면 사용료만 면제. `directPlatformFeeEnabled`는 스냅샷이 없는 기존 지정 의뢰의 이벤트 스위치로만 남긴다.
  ...
  - 설정: 단가·신속비=`AdminCreditSettingsTab` / `PATCH /api/admin/settings/credits`. 하청 % · 플랫폼 사용료 on/% = `DevopsPlatformFeeTab` / `PATCH /api/admin/settings/platform-fees`.
...
- 수익 계정 SSOT:
  ...
  - **딜러십(salesman) 파트너 조건**: 영업 수수료=심플웨이·커스텀어벗(**기공 제외**, 배송비·월정액 제외). **요율 20% 고정**(`dealershipActiveCommissionRate`). **이미 유치한 의뢰자는 유치(가입·재귀속) 당시 요율 유지**(`BusinessAnchor.dealershipCommissionRate` 스탬프 · `dealershipCommissionRateLog`). **90일 무주문이면 소개 리셋**(`referredByAnchorId` 해제). 리셋 후 재유치하면 20%를 스탬프. 배송비=수신자(치과 또는 기공소) 부담. 관리자「플랫폼 설정 · 분배 비율」딜러십. `resolveDealershipRateForAcquiredAt`. 시계=`max(referralAssignedAt || BA.createdAt, 최근 Request.createdAt, 최근 소비 occurredAt)`, KST 일자. 코드 없이 가입하면 귀속은 비워 두고, 대표가 설정-사업자에서 영업자 코드를 등록한다(`POST /api/businesses/me/apply-referral`, 대표만). 잡=`jobs/dailyReferralOwnershipResetWorker.js`. 과거 `REV_SALESMAN` 불변.
  - **배송 분배**: 사업영역 분배 재원에서 제외. 매출에서 배송비를 먼저 차감한 나머지만 분배. 제조사 배송 매입가(부가세 포함)·고객 배송비 잔여는 출고 장부 흐름.
  - **플랫폼 분배**: 하청 수수료(기본 10%)와 플랫폼 사용료(지정·협력, 그리고 하청에 더하는 요율. 학습 이용 허용 스냅샷이면 사용료 0)를 어벗츠 90% / 개발운영사 10%(비율 수정 가능). 개발운영사 장부=포함가·지급=잔액 그대로.
...
- 커스텀 어벗 의뢰 단가 SSOT: 관리자「플랫폼 설정 · 커스텀어벗」. **정상가**=`membershipProductionPrice`(기본 **13,000원**). **런칭 이벤트**=`customAbutmentLaunchEventProductionPrice`(기본 **10,000원**) · 관리자 UI는 **on/off만**(시작·종료일 입력 없음) · **변경은 내일 0시(KST)부터**(`customAbutmentLaunchEventChangeScheduled*`, 분배 비율과 동일) — `resolveCustomAbutmentProductionPriceForAt`(의뢰 생성·hold 시점). **신규 Request는 항상 생산만**(`custom_abutment`). `design_custom_abutment`·`membershipDesignAndProductionPrice`(옛 2.5만)는 **레거시 읽기 전용**. 기공의뢰 CA 디자인은 수주 기공소·`labFeeSchedule` 커스텀어벗 수가. 신속=`expressFee`(기본 **+2,000원**). 배송: 이벤트=박스당 `shippingFee` · 정상가=박스당 **또는**(기공소만) FM덴탈 월정액(`fmDentalMonthlyShippingFee`, BA.`fmDentalShippingActive` 시 `SHIPPING_SPEND_*` 스킵, 원장 `FM_DENTAL_SHIPPING_SPEND`; **기공소만 가입**, 정책 안내·대표만). `regular*`·관리자「멤버/일반」은 **딜러 유무 분배**용. 치과 멤버십 월정 없음.
- 롤백 원칙:
