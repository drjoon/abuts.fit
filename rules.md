...
- 포장.발송 화면의 기본 액션은 **`택배 접수`** 이며, 접수 시 현재 우편함 구성으로 **운송장 출력/저장까지 함께 처리**합니다.
- 운송장 출력은 독립 메인 액션이 아니라 **접수 결과물**입니다. 단독 출력 버튼보다 `택배 접수`, `택배 접수 취소`, `접수 내용 수정` 흐름을 우선합니다.
- 부분 유실/누락이 생기면 우편함에서 제품을 제외하거나 박스 전체를 다음날로 미룬 뒤, **같은 우편함을 다시 접수**하여 해당 라벨만 재출력합니다.
