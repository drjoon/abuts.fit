...
- 2026-02부터 Guide Tour 기능은 더 이상 사용하지 않으며 관련 코드/문서는 제거합니다.
...
- 재의뢰 판정 기준: 동일 치과/환자/치아 조합의 의뢰가 **최근 90일 내 존재**하고,
  해당 의뢰가 **취소 상태가 아닌 경우** 재의뢰로 분류하여 10,000원 고정가를 적용합니다.
...
### 6.4.1 발송 예정일(Shipping ETA) 공휴일 이월 및 재조정 규칙

- **SSOT**: 발송 예정일(`timeline.originalEstimatedShipYmd`, `timeline.nextEstimatedShipYmd`, `shipping-estimate`, `bulk-shipping`의 ETA) 계산은 백엔드에서 일원화한다.
...
#### 6.8.1 아키텍처 (로컬 스토리지 SSOT)

**데이터 저장소:**

- **로컬 스토리지**: 파일 메타데이터, CaseInfos, 중복 처리 결정 저장
- **IndexedDB**: 실제 File 객체 저장 (용량 제한 회피)
- **백엔드**: 의뢰하기 클릭 시에만 S3 업로드 및 Draft 생성

**핵심 파일:**

- `localDraftStorage.ts`: 로컬 스토리지 SSOT 유틸리티
- `fileIndexedDB.ts`: IndexedDB File 저장 유틸리티
- `useLocalDraft.ts`: 로컬 Draft 관리 훅
- `useNewRequestV3.ts`: 새 의뢰 페이지 메인 로직
- `NewRequestPageV3.tsx`: 새 의뢰 페이지 컴포넌트

#### 6.8.2 중복 체크 워크플로우

**1단계: 파일 드롭 시 (로컬 중복 체크)**

- 로컬 스토리지의 파일 목록과 비교하여 중복 파일 건너뛰기
- 중복되지 않은 파일은 로컬 스토리지 + IndexedDB에만 저장
- **백엔드 업로드 없음** (S3 업로드 X, Draft 생성 X)
- 구현: `useLocalDraft.ts`의 `addFiles` 함수

**2단계: 정보 입력 시 (로컬 저장 + 백엔드 중복 체크)**

- 정보 입력 시 로컬 스토리지의 `caseInfosMap` 업데이트
- 환자명, 치과명, 치아번호가 모두 입력되면 자동으로 백엔드 `/api/requests/my/check-duplicate` 호출
- 중복 발견 시 모달 표시하여 사용자에게 선택지 제공 (replace/remake/skip)
- 사용자 선택 내용은 로컬 스토리지의 `duplicateResolutions`에 저장
- 구현: `useNewRequestV3.ts`의 `handleUpdateInfo` 및 `checkDuplicate` 함수

**3단계: 의뢰하기 클릭 시 (백엔드 업로드 + 제출)**

- 로컬 스토리지 데이터를 기반으로 백엔드 처리 시작
- Draft 생성 → 각 파일별 S3 업로드 → Draft에 파일 추가 → 의뢰 생성
- 중복 처리 결정(`duplicateResolutions`)을 함께 전달
- 성공 시 로컬 스토리지 및 IndexedDB 초기화
- 구현: `useNewRequestV3.ts`의 `handleSubmit` 함수

#### 6.8.2 중복 의뢰 감지 기준

- 동일 `치과명(clinicName)` + `환자명(patientName)` + `치아번호(tooth)` 조합이 기존 의뢰에 존재
- 기존 의뢰의 상태가 `취소`가 아닌 경우

#### 6.8.3 중복 처리 전략

**replace (새 의뢰로 교체)**

- 적용 조건: 기존 의뢰가 **의뢰(Request) 또는 CAM 단계**일 때 (stageOrder < 2)
- 기존 의뢰를 취소하고 새로운 의뢰로 대체
- 기존 의뢰는 취소되고 크레딧 환불 처리

**remake (하나 더 의뢰하기)**

- 적용 조건: 기존 의뢰가 **가공(Machining) 단계 이후**일 때 (stageOrder >= 2)
- 기존 의뢰를 그대로 두고 새로운 의뢰를 추가로 생성
- 기존 의뢰는 계속 진행되며, 새 의뢰가 별도로 생성됨

**skip (기존 유지)**

- 중복 파일을 무시하고 기존 의뢰만 유지
- 새로운 의뢰를 생성하지 않음

#### 6.8.4 파일 삭제 시 로컬 스토리지 동기화

- 파일 삭제 시 로컬 스토리지에서도 해당 파일 제거
- 재업로드 시 중복으로 인식되지 않도록 함
- 구현: `useNewRequestFilesV2.ts`의 `handleRemoveFile` 함수

### 6.9 영업자 리퍼럴 수수료

- **직접리퍼럴** (영업자 A → 의뢰자 B 소개): B의 지난달 유료 매출의 **5%**를 A에게 수수료로 적립한다.
- **간접리퍼럴** (영업자 A → 영업자 C 소개 → 의뢰자 D 소개): D의 지난달 유료 매출의 **2.5%**를 A에게 추가 수수료로 적립한다.
- 의뢰자→의뢰자 소개는 수수료 없음 (할인 가격 결정에만 사용).
- 영업자 간 계층은 1단계까지만 적용하며, 더 깊은 레벨/중복 수수료는 없다.
- 수수료 계산 기준은 **지난달(lastMonth)** 완료 주문의 유료 매출(paidAmount)이다.

### 6.9.1 리퍼럴 코드 가입 규칙

- 의뢰자(Requestor) 가입 시 리퍼럴 코드 입력은 **필수**이다.
- 가입 완료 후 `referredByUserId`(추천인)는 **변경 불가**이다.
- 추천인은 활성 상태의 의뢰자 또는 영업자 계정만 가능하다.

### 6.10 의뢰자 리퍼럴 코드/할인(사업자 단위)

- 의뢰자(Requestor)의 리퍼럴 코드는 **base36(대문자 영문 + 숫자) 5자리**를 사용한다.
- 영업자(Salesman)의 리퍼럴 코드는 **base36(대문자 영문 + 숫자) 4자리**를 사용한다.
- 리퍼럴로 발생하는 가격 할인/주문량 집계 등 보상은 **사업자(RequestorOrganization) 단위**로 적용한다.
  - 직원 이메일로 가입이 발생해도 할인/집계는 직원 개인이 아니라 해당 사업자에 귀속된다.

#### 6.10.1 리퍼럴 직계 멤버 정의

- 직계 멤버 목록은 **내가 속한 사업자 + 내 하위 사업자** 기준으로 구성한다.
- 의뢰자: **내가 속한 사업자 + 내 하위 사업자**.
- 영업자: **내가 소개한 의뢰자 + 내 하위 영업자**.
- 직계 멤버 주문량 집계는 **사업자 단위**(사업자가 없는 의뢰자는 사용자 단위)로 계산한다.

### 6.11 가상 우편함(Virtual Mailbox) 시스템

**구조 및 주소 형식**:

- 선반(Shelf): A~X (총 24개, 3개씩 묶음: A-C, D-F, ... V-X)
- 선반 행(ShelfRow): 1~4 (위에서부터 1번, 맨 아래가 4번)
- 빈 열(BinCol): A, B, C, D (4개 열)
- 빈 행(BinRow): 1~4 (4개 행)
- **주소 형식**: `{Shelf}{ShelfRow}{BinCol}{BinRow}` (예: A1A1, C4D4)
- **총 용량**: 24 × 4 × 4 × 4 = 1,536개 박스

**할당 및 해제 정책**:

- 세척.패킹 단계 승인 시: 자동으로 빈 우편함 주소 할당 (`allocateVirtualMailboxAddress()`)
- 포장.발송 단계 롤백 시: 할당된 우편함 주소 해제 (`mailboxAddress = null`)
- 발송 등록 시: 우편함의 모든 의뢰건이 함께 처리되며, 처리 후 추적관리 단계로 이동

**UI 표현**:

- 포장.발송 페이지: 선반 그룹 선택 버튼(A-C, D-F 등)으로 3개 선반씩 표시
- 각 선반행(예: A1)은 16개 빈(4×4)을 한 덩어리로 표시
- 빈 상단: 주소 라벨 (예: A1A1)
- 빈 중앙: 해당 주소에 할당된 의뢰건 개수
- 빈 클릭 시: 모달로 해당 우편함의 모든 의뢰 내용 표시
...
- 이 저장소는 아래 배포 단위로 분리 운영합니다.
  - `web/`: 백엔드 API + 프론트 서빙
  - `background/`: (레거시) 참고용으로만 유지
  - `lambda/`: AWS Lambda 함수
- **기본 원칙**: DB 모델(스키마)은 하나의 소스에서 관리합니다.
  - 주 소스: `web/backend/models`
  - `background/models`는 **레거시 참고용 복사본**이며, 더 이상 동기화/리팩터링 대상이 아닙니다.
  - lambda 등 다른 프로세스는 각 환경의 운영 정책에 따라 별도 관리합니다.

### 11.x background 레거시 정책

- `/background` 폴더는 **더 이상 운영하지 않는 레거시**이며, 코드/스키마/정책 반영 대상이 아니다.
- 참고를 위해 남겨두는 용도이며, 앞으로 리팩터링/마이그레이션/필드 rename은 `web/*` 기준으로만 진행한다.

### 11.1 (레거시) DB 모델 복사 원칙

- 과거에는 `background/models`를 `web/backend/models`에서 복사해 동기화했으나,
  현재 `/background`는 레거시이므로 해당 운영 규칙은 더 이상 적용하지 않는다.
