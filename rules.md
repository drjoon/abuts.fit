...
- 추적관리를 제외한 나머지 공정 단계(의뢰, CAM, 가공, 세척.패킹, 포장.발송)의 의뢰건이 하나라도 있으면서 크레딧이 `minCreditForRequest` 미만이면 의뢰불가 경고를 표시합니다.
- 포장.발송 단계의 의뢰건이 있으면서 크레딧이 `shippingFee` 미만이면 배송불가 경고를 표시합니다.
...
- 의뢰카드의 테두리 색상과 뱃지 배경으로 마감까지 남은 영업일을 시각화합니다.
...
- 카드 테두리: 마감 시간 기반 색상 적용
- 뱃지: 카드 오른쪽 아래에 "3일 2시간" 형식으로 남은 영업일 표시 (뱃지 배경에 동일 색상)
- 구현 파일: `WorksheetCardGrid.tsx`, `deadline.ts` 유틸리티
