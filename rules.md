...
- 운송장 출력은 접수와 분리된 독립 단계입니다. 출력 후 아직 접수되지 않은 우편함은 `printed` 상태로 유지해야 하며, 사용자는 이 상태에서 제품 제외/보류/재출력을 반복할 수 있어야 합니다.
- 창고 실물과 라벨이 맞으면 박스에 담고 라벨을 붙입니다. 맞지 않는 우편함은 박스에 담지 않고 다시 우편함으로 돌려놓은 뒤, 웹앱에서 실제 창고 기준으로 우편함 구성을 수정합니다.
- 웹앱 수정 시 중복 제품이 있으면 하나만 남기고 제거하며, 누락 제품이 있으면 **부분 발송(누락 제품 제외 후 나머지 접수)** 또는 **우편함 전체를 오늘 미발송 처리 후 다음날 추가 발송** 중 제조사 직원이 선택합니다.
- 부분 유실/누락/추가 발견이 생기면 우편함 구성을 다시 수정하고, 필요 시 **같은 우편함 라벨을 재출력한 뒤 다시 접수**할 수 있어야 합니다.
- `포장.발송` UI는 최소한 `운송장 출력/저장`, `택배 접수`, `택배 접수 취소`, `내용 수정 후 재출력/재접수` 흐름을 분리해 표현해야 합니다.
- `추적관리` 조회 화면에 노출되더라도 **집하완료 전이면 롤백/개별 취소/전체 취소가 가능**해야 하며, 동일 송장/우편함 기준으로 취소 대상을 묶어 처리합니다.
- 운송장 재출력은 **부분 출력**을 지원해야 하며, 우편함별 선택을 기준으로 2번째 출력부터 필요한 박스만 다시 출력합니다.
...
- 운영 기준 시각은 **오후 2시 라벨 출력, 오후 4시 집하(code 11)** 입니다. `accepted`는 접수 완료, `picked_up`는 집하 완료, `completed`는 최종 배송 완료를 의미합니다.
