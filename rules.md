...
- 수익 계정 SSOT:
  ...
  - **딜러십(salesman) 파트너 조건**: 영업 수수료=심플웨이·커스텀어벗(**기공 제외**, 배송비·월정액 제외). **신규 유치 요율**(기본 **20%**, `dealershipActiveCommissionRate`). 관리자가 예약하면 해당일 0시(KST)부터 신규 유치만 **15%·10%**로 인하. **이미 유치한 의뢰자는 유치(가입·재귀속) 당시 요율 유지**(`BusinessAnchor.dealershipCommissionRate` 스탬프 · `dealershipCommissionRateLog`). **90일 무주문이면 소개 리셋**(`referredByAnchorId` 해제). 리셋 후 재유치하면 그 시점 요율을 스탬프. 배송비=수신자(치과 또는 기공소) 부담. 관리자「플랫폼 설정 · 분배 비율」딜러십. `resolveDealershipRateForAcquiredAt`. 시계=`max(referralAssignedAt || BA.createdAt, 최근 Request.createdAt, 최근 소비 occurredAt)`, KST 일자. 코드 없이 가입하면 귀속은 비워 두고, 대표가 설정-사업자에서 영업자 코드를 등록한다(`POST /api/businesses/me/apply-referral`, 대표만). 잡=`jobs/dailyReferralOwnershipResetWorker.js`. 과거 `REV_SALESMAN` 불변.
