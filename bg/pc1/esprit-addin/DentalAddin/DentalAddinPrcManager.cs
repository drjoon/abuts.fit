...
namespace Abuts.EspritAddIns.ESPRIT2025AddinProject.DentalAddin
{
    ...
    public class DentalAddinPrcManager
    {
        ...
        public bool ApplyBackendPrcNames(BackendApiClient.RequestMetaCaseInfos requestMeta, string requestId, string implantLabel)
        {
            ...
            {
                AppLogger.Log($"DentalAddinPrcManager.ApplyBackendPrcNames: PRC 파일명 누락 ({context}) - face={faceName}, connection={connectionName}");
                throw new InvalidOperationException($"Backend PRC file name is missing ({context})");
            }

            if (!TryResolveBackendPrcPath("7_FrontFace prc", faceName, out string facePath))
            {
                AppLogger.Log($"DentalAddinPrcManager.ApplyBackendPrcNames: FaceHole PRC resolve failed ({context}) - fileName={faceName}");
                throw new InvalidOperationException($"FaceHole PRC resolve failed ({context}) - fileName={faceName}");
            }

            if (!TryResolveBackendPrcPath("2_Connection prc", connectionName, out string connectionPath))
            {
            ...
            AppLogger.Log($"DentalAddinPrcManager.ApplyBackendPrcNames: FaceHole={Path.GetFileName(facePath)}, Connection={Path.GetFileName(connectionPath)}");
            return true;
        }
        ...
        private static bool TryResolveBackendPrcPath(string subDir, string fileName, out string resolved)
        {
            ...
            {
                AppLogger.Log($"DentalAddinPrcManager.TryResolveBackendPrcPath: 경로 조합 실패 - {ex.GetType().Name}:{ex.Message}");
                return false;
            }
            if (File.Exists(resolved))
            {
                AppLogger.Log($"DentalAddinPrcManager.TryResolveBackendPrcPath: PRC 확인 완료 - subDir={subDir}, file={fileName}, resolved={resolved}");
                return true;
            }
            ...
            try
            {
                ...
                {
                    AppLogger.Log($"DentalAddinPrcManager.TryResolveBackendPrcPath: PRC 디렉터리 없음 - dir={baseDirectory}, file={fileName}");
                    return false;
                }
                ...
                foreach (string candidatePath in Directory.GetFiles(baseDirectory, "*.prc", SearchOption.TopDirectoryOnly))
                {
                    ...
                    if (string.Equals(candidateName, targetName, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        AppLogger.Log($"DentalAddinPrcManager.TryResolveBackendPrcPath: 정규화 fallback 매칭 성공 - requested={fileName}, resolved={resolved}");
                        return true;
                    }
                }

                AppLogger.Log($"DentalAddinPrcManager.TryResolveBackendPrcPath: PRC 파일 없음 - requested={fileName}, attempted={resolved}, dir={baseDirectory}");
                return false;
            }
            catch (Exception ex)
            {
                AppLogger.Log($"DentalAddinPrcManager.TryResolveBackendPrcPath: fallback 탐색 실패 - {ex.GetType().Name}:{ex.Message}");
                return false;
            }
        }
