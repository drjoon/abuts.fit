...
namespace DentalAddin
{
    internal sealed partial class MainModule
    {
        ...
        /// TopZLimit은 FACE.prc 값 유지(코드에서 덮어쓰지 않음).
        /// </summary>
        ...
        /// - TopZ: FACE.prc TopZLimit 유지(코드 미개입) / RL=1: BottomZ=-RightX / RL=2: BottomZ=+RightX
        ...
        private static void AddSplitOp(string region, string angleLabel, int boundaryKey, FreeFormFeature freeFormFeature, string prcFile, TechnologyUtility technologyUtility, bool applyBoundary)
        {
            ...
            if (tech[0] is TechLatheMoldRoughing roughing)
            {
                ...
                else
                {
                    ...
                    // 요청 반영:
                    // MillRough_3D*.prc 기술 파라미터(증분 깊이/절삭 속도/공차/코너값/BottomZLimit 포함)는
                    // 코드에서 오버라이드하지 않는다(rules.md §4.10).
                    // 2026-08-30 감사: BottomZLimit=-2.2를 LastAppliedMoveDeltaX(+~11)로 시프트하면
                    // 샘플 기준 -2.2→+8.795로 크게 바뀌어 현재 정상 가공과 충돌 → 보정 코드 되돌림.
                    // 검색: TryShiftRoughBottomZLimitByMoveDelta(제거됨), LastAppliedMoveDeltaX
