...
namespace DentalAddin
{
    internal sealed partial class MainModule
    {
        ...
        private static string ResolveCompositeFinishCommonToolId()
        {
            try
            {
                ...
                // 1) 명시 ID 우선
                foreach (Tool tool in (System.Collections.IEnumerable)Document.Tools)
                {
                    ...
                    if (string.Equals(tool.ToolID.Trim(), CompositeFinishCommonToolId, StringComparison.OrdinalIgnoreCase))
                    {
                        return tool.ToolID;
                    }
                }

                // 2) D1.2 BallMill로 fallback
                foreach (Tool tool in (System.Collections.IEnumerable)Document.Tools)
                {
                    if (tool == null || string.IsNullOrWhiteSpace(tool.ToolID))
                    {
                    ...
                    if (Math.Abs(ball.ToolDiameter - CompositeFinishCommonToolDiameterMm) <= 0.01)
                    {
                        return tool.ToolID;
                    }
                }
            ...
            {
                DentalLogger.Log($"Composite2SplitLine2 - Finish 공통 공구(D1.2) 탐색 실패: {ex.GetType().Name}:{ex.Message}");
            }

            return null;
        }

        private static void TryApplyCompositeFinishCommonTool(TechLatheMill5xComposite op, string label)
        {
            if (op == null)
            {
                return;
            ...
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} Finish 공통 공구(D1.2) 미발견: 기존 ToolID 유지('{op.ToolID ?? ""}')");
                return;
            }

            string before = op.ToolID;
            op.ToolID = finishToolId;
            DentalLogger.Log($"Composite2SplitLine2 - {label} Finish 공통 공구 적용: '{before ?? ""}' -> '{op.ToolID}'");
        }
        ...
        private static bool TryRunComposite2SplitABCore(FreeFormFeature freeFormFeature)
        {
            ...
            // Finish_Front 시작점: FirstPassPercent=0 (요청 SSOT).
            // tip 부착은 Add 후 툴패스 강제 재생성(TryForceCompositeToolpathRegenAfterAdd)으로 UI 확인과 동일하게 맞춘다.
            ...
            // Finish_Front 시작점 정책:
            // - 기본값: FirstPassPercent=0 (UI 0 재입력+확인과 동일 SSOT)
            // - env(ABUTS_COMPOSITE_FIRST_PASS_PERCENT_A) 지정 시 env(퍼센트) 우선
            // - X 기반 baseAFirstPercent는 로그/가드용; 기본 적용값은 0
            ...
            // Finish_Front/Back는 각각 독립 dedicated DriveSurface(Project.igs merge)를 사용한다.
            // AuxDrive(SurfaceNumber2, XT/ZT 변환)는 Finish PassPercent(절대 X/20 스케일)와
            // 맞지 않아 툴패스가 커넥션 쪽으로 밀린다. (2026-08-25 회귀)
            int dedicatedAKey = 0;
            int dedicatedBKey = 0;
            ...
            // Finish_* 공통 공구 정책: D1.2 볼엔드밀(BM_D1.2) 강제
            TryApplyCompositeFinishCommonTool(opA, "A");
            if (runB && opB != null)
            {
            ...
            // DriveSurface/공구/공차 바인딩 이후 PassPercent를 재커밋한다.
            // 증상: FirstPass%=0이 UI에 보이지만 툴패스는 PRC 기본(≈3%)처럼 모델 tip에서 떨어짐.
            // UI에서 0을 다시 입력+확인하면 정상 → dirty 트리거(nudge) + DriveSurface 이후 재적용이 필요.
            double committedAFirst = opA.FirstPassPercent;
            double committedALast = opA.LastPassPercent;
            double committedBFirst = (runB && opB != null) ? opB.FirstPassPercent : 0.0;
            double committedBLast = (runB && opB != null) ? opB.LastPassPercent : 0.0;
            if (runA)
            {
                TryCommitCompositePassPercents(opA, committedAFirst, committedALast, "A-preAdd");
            }
            if (runB && opB != null)
            {
                TryCommitCompositePassPercents(opB, committedBFirst, committedBLast, "B-preAdd");
            }
            ...
            if (runA)
            {
                ...
                // UI에서 첫패스 0 재입력+확인과 동일: 속성 재커밋 + 툴패스 강제 재생성(+필요시 Remove/ReAdd)
                TryForceCompositeToolpathRegenAfterAdd(
                    beforeAddCountBaseA,
                    opA,
                    freeFormFeature,
                    committedAFirst,
        ...
        /// <summary>
        /// DriveSurface 바인딩 이후 First/LastPassPercent를 dirty-트리거(nudge) 후 재커밋한다.
        /// ESPRIT는 FirstPass%=0을 속성만 저장하고 툴패스 재계산을 건너뛰는 경우가 있음
        /// (UI에서 동일 값 재입력+확인 시 정상 재계산).
        /// </summary>
        private static void TryCommitCompositePassPercents(
            TechLatheMill5xComposite op,
            ...
            string label)
        {
            ...
            double first = Clamp(firstPassPercent, 0.0, 100.0);
            double last = Clamp(lastPassPercent, first, 100.0);

            try
            {
                op.PassPosition = espMill5xCompositePassPosition.espMill5xCompositePassPositionStartEndPosition;
            }
            catch
            {
            }

            // 목표값이 0일 때 특히 dirty가 안 생기므로 미세 nudge 후 목표값으로 되돌린다.
            double nudgeFirst = first <= 1e-9
                ? Math.Min(0.01, Math.Max(0.0, last - 1e-6))
                : Math.Max(0.0, first - 0.01);
            if (Math.Abs(nudgeFirst - first) < 1e-12)
            {
                nudgeFirst = Math.Min(first + 0.01, Math.Max(0.0, last - 1e-6));
            }

            try
            {
                op.FirstPassPercent = nudgeFirst;
                op.LastPassPercent = last;
                op.FirstPassPercent = first;
                op.LastPassPercent = last;
                DentalLogger.Log(
                    $"Composite2SplitLine2 - {label} PassPercent 재커밋: nudgeFirst={nudgeFirst.ToString("F4", CultureInfo.InvariantCulture)} -> First={op.FirstPassPercent.ToString("F4", CultureInfo.InvariantCulture)}, Last={op.LastPassPercent.ToString("F4", CultureInfo.InvariantCulture)}");
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} PassPercent 재커밋 실패: {ex.GetType().Name}:{ex.Message}");
            }
        }

        private static void TryCommitCompositePassPercentsOnAddedOperation(
            int beforeCount,
            double firstPassPercent,
            double lastPassPercent,
            string label)
        {
            try
            {
                ...
                int afterCount = Document.Operations.Count;
                ...
                object opObj = null;
                try { opObj = Document.Operations[afterCount]; } catch { }
                if (opObj == null)
                {
                    DentalLogger.Log($"Composite2SplitLine2 - {label} PassPercent postAdd 실패: Operations[{afterCount}] null");
                    return;
                }

                object tech = TryGetOperationTechnology(opObj);
                if (tech == null)
                {
                    DentalLogger.Log($"Composite2SplitLine2 - {label} PassPercent postAdd 실패: Operation.Technology 미해결");
                    return;
                }

                TechLatheMill5xComposite composite = tech as TechLatheMill5xComposite;
                if (composite != null)
                {
                    TryCommitCompositePassPercents(composite, firstPassPercent, lastPassPercent, label);
                    return;
                }

                // 캐스팅 실패 시 reflection으로 First/LastPassPercent만 재적용
                TryCommitCompositePassPercentsViaReflection(tech, firstPassPercent, lastPassPercent, label);
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} PassPercent postAdd 예외: {ex.GetType().Name}:{ex.Message}");
            }
        }

        /// <summary>
        /// UI에서 첫 패스%를 0으로 다시 입력+확인하면 툴패스가 재생성되어 tip에 붙는 동작을 모사한다.
        /// 속성만 재설정해서는 dirty/재생성이 안 되므로:
        /// 1) 큰 폭 FirstPass dirty(3→0) + StepIncrement touch
        /// 2) Rebuild 계열 호출
        /// 3) 실패 시 Remove→PassPercent 재커밋→ReAdd (대화상자 OK로 경로 재생성과 가장 유사)
        /// </summary>
        private static void TryForceCompositeToolpathRegenAfterAdd(
            int beforeCount,
            TechLatheMill5xComposite sourceTech,
            ...
            string nameSuffix)
        {
            try
            {
                ...
                if (opObj == null)
                {
                    DentalLogger.Log($"Composite2SplitLine2 - {label} toolpath regen 실패: Operations[{afterCount}] null");
                    return;
                }
                ...
                object liveTech = TryGetOperationTechnology(opObj) ?? (object)sourceTech;
                if (liveTech == null)
                {
                    TryCommitCompositePassPercentsOnAddedOperation(beforeCount, first, last, label);
                    DentalLogger.Log($"Composite2SplitLine2 - {label} toolpath regen: Technology 미해결, PassPercent 재커밋만 수행");
                    return;
                }

                // 1) UI 대화상자처럼 FirstPass를 눈에 띄게 바꿨다가 0으로 되돌린다 (0.01 nudge는 dirty 미발생).
                //    PRC 구기본(3) → 목표(0) 전환이 재계산 트리거에 가장 가깝다.
                double dirtyFirst = Clamp(Math.Max(3.0, first + 3.0), 0.0, Math.Max(0.0, last - 1e-6));
                if (Math.Abs(dirtyFirst - first) < 1e-9)
                {
                    dirtyFirst = Clamp(first + 1.0, 0.0, Math.Max(0.0, last - 1e-6));
                }

                TrySetNumericProperty(liveTech, "FirstPassPercent", dirtyFirst);
                TryNudgeNumericProperty(liveTech, "StepIncrement", label);
                TrySetNumericProperty(liveTech, "LastPassPercent", last);
                TrySetNumericProperty(liveTech, "FirstPassPercent", first);
                TrySetNumericPropertyDynamic(liveTech, "FirstPassPercent", first);
                TrySetNumericPropertyDynamic(liveTech, "LastPassPercent", last);

                TechLatheMill5xComposite composite = liveTech as TechLatheMill5xComposite;
                if (composite != null)
                {
                    TryCommitCompositePassPercents(composite, first, last, label);
                }
                else
                {
                    TryCommitCompositePassPercentsViaReflection(liveTech, first, last, label);
                }

                // 2) Operation/Document Rebuild 계열
                bool rebuilt = TryInvokeRebuildMethods(opObj, label + ":Operation");
                if (!rebuilt && Document != null)
                {
                    rebuilt = TryInvokeRebuildMethods(Document, label + ":Document");
                }

                // 3) Rebuild가 없으면 Remove→ReAdd로 툴패스 자체를 다시 생성.
                //    ReAdd는 FirstPass=dirty(3)로 만든 뒤 live에서 0으로 바꿔 UI의 "값 변경+확인"과 같게 한다.
                //    (처음부터 0으로 ReAdd하면 첫 Add와 동일하게 tip에서 떨어지는 증상 재현)
                bool readded = false;
                if (!rebuilt && sourceTech != null && freeFormFeature != null)
                {
                    readded = TryRemoveAndReAddCompositeOperation(
                        afterCount,
                        sourceTech,
                        freeFormFeature,
                        first,
                ...
                DentalLogger.Log(
                    $"Composite2SplitLine2 - {label} toolpath regen 완료: rebuilt={rebuilt}, readded={readded}, dirtyFirst={dirtyFirst.ToString("F4", CultureInfo.InvariantCulture)}, First={first.ToString("F4", CultureInfo.InvariantCulture)}, Last={last.ToString("F4", CultureInfo.InvariantCulture)}");
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} toolpath regen 예외: {ex.GetType().Name}:{ex.Message}");
            }
        }

        private static bool TryRemoveAndReAddCompositeOperation(
            int opIndex,
            TechLatheMill5xComposite sourceTech,
            FreeFormFeature freeFormFeature,
            double firstPassPercent,
            double lastPassPercent,
            double dirtyFirstPassPercent,
            string label,
            string nameSuffix)
        {
            try
            {
                if (Document?.Operations == null || sourceTech == null || freeFormFeature == null || opIndex <= 0)
                {
                    return false;
                }
                ...
                double first = Clamp(firstPassPercent, 0.0, 100.0);
                double last = Clamp(lastPassPercent, first, 100.0);
                double dirtyFirst = Clamp(dirtyFirstPassPercent, 0.0, Math.Max(0.0, last - 1e-6));
                if (Math.Abs(dirtyFirst - first) < 1e-9)
                {
                    dirtyFirst = Clamp(first + 3.0, 0.0, Math.Max(0.0, last - 1e-6));
                }

                try
                {
                    Document.Operations.Remove(opIndex);
                }
                catch (Exception rmEx)
                {
                    DentalLogger.Log($"Composite2SplitLine2 - {label} Remove 실패: {rmEx.GetType().Name}:{rmEx.Message}");
                    return false;
                }

                // ReAdd는 dirty FirstPass로 생성 → live에서 목표 0으로 전환 (대화상자 값 변경+확인)
                TryCommitCompositePassPercents(sourceTech, dirtyFirst, last, label + ":preReAddDirty");

                int beforeReAdd = Document.Operations.Count;
                TryAddOperation(sourceTech, freeFormFeature, "Composite2SplitLine2:ReAdd:" + label);
                if (!string.IsNullOrWhiteSpace(nameSuffix))
                {
                    TryAppendCompositeSuffixToNewOperations(beforeReAdd, nameSuffix);
                }

                TryCommitCompositePassPercentsOnAddedOperation(beforeReAdd, first, last, label + ":reAddToTarget");

                // live tech에 3→0을 한 번 더 명시적으로 적용
                int afterReAdd = Document.Operations.Count;
                if (afterReAdd > beforeReAdd)
                {
                    object reAddOp = null;
                    try { reAddOp = Document.Operations[afterReAdd]; } catch { }
                    object liveTech = TryGetOperationTechnology(reAddOp);
                    if (liveTech != null)
                    {
                        TrySetNumericProperty(liveTech, "FirstPassPercent", dirtyFirst);
                        TrySetNumericProperty(liveTech, "FirstPassPercent", first);
                        TrySetNumericPropertyDynamic(liveTech, "FirstPassPercent", first);
                        TryInvokeRebuildMethods(reAddOp, label + ":reAddOp");
                    }
                }

                DentalLogger.Log(
                    $"Composite2SplitLine2 - {label} Remove+ReAdd 완료 (beforeReAdd={beforeReAdd}, after={Document.Operations.Count}, dirtyFirst={dirtyFirst.ToString("F4", CultureInfo.InvariantCulture)} -> First={first.ToString("F4", CultureInfo.InvariantCulture)})");
                return Document.Operations.Count > beforeReAdd;
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} Remove+ReAdd 예외: {ex.GetType().Name}:{ex.Message}");
                return false;
            }
        }

        private static void TryNudgeNumericProperty(object target, string propertyName, string label, double? restoreTo = null)
        {
            ...
            try
            {
                object raw = target.GetType().InvokeMember(
                    propertyName,
                    BindingFlags.GetProperty,
                    null,
                    target,
                ...
                double current = Convert.ToDouble(raw, CultureInfo.InvariantCulture);
                double nudged = current + 0.001;
                double restore = restoreTo ?? current;
                ...
                DentalLogger.Log($"Composite2SplitLine2 - {label} {propertyName} nudge: {current.ToString("F6", CultureInfo.InvariantCulture)} -> {nudged.ToString("F6", CultureInfo.InvariantCulture)} -> {restore.ToString("F6", CultureInfo.InvariantCulture)}");
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} {propertyName} nudge 실패: {ex.GetType().Name}:{ex.Message}");
            }
        }

        private static void TrySetNumericProperty(object target, string propertyName, double value)
        {
            if (target == null || string.IsNullOrWhiteSpace(propertyName))
            {
                return;
        ...
        private static void TrySetNumericPropertyDynamic(object target, string propertyName, double value)
        {
            if (target == null || string.IsNullOrWhiteSpace(propertyName))
            {
                return;
            ...
            try
            {
                ...
                switch (propertyName)
                {
                    case "FirstPassPercent":
                        dyn.FirstPassPercent = value;
                        break;
                    case "LastPassPercent":
                        dyn.LastPassPercent = value;
                        break;
                    case "StepIncrement":
                        dyn.StepIncrement = value;
                        break;
                }
            }
            catch
        ...
        private static bool TryInvokeRebuildMethods(object target, string label)
        {
            if (target == null)
            {
                return false;
            ...
            {
                if (TryInvokeNamedMethod(target, methodName, null, label))
                {
                    return true;
                }

                if (TryInvokeNamedMethod(target, methodName, new object[] { false }, label))
                {
                    return true;
                }

                if (TryInvokeNamedMethod(target, methodName, new object[] { true }, label))
                {
                    return true;
                }
            }

            // COM 실체에서 Calc/Regen 계열 메서드명을 한번 스캔해 로그하고 호출 시도
            try
            {
                ...
                {
                    if (mi == null || string.IsNullOrWhiteSpace(mi.Name))
                    {
                        continue;
                    }

                    string name = mi.Name;
                    if (name.IndexOf("Calc", StringComparison.OrdinalIgnoreCase) < 0
                        && name.IndexOf("Rebuild", StringComparison.OrdinalIgnoreCase) < 0
                        && name.IndexOf("Regen", StringComparison.OrdinalIgnoreCase) < 0
                        && name.IndexOf("Toolpath", StringComparison.OrdinalIgnoreCase) < 0
                        && name.IndexOf("ToolPath", StringComparison.OrdinalIgnoreCase) < 0)
                    {
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        DentalLogger.Log($"Composite2SplitLine2 - {label} reflected {name}() 호출 성공");
                        return true;
                    }
                    catch (Exception ex)
                    {
                        DentalLogger.Log($"Composite2SplitLine2 - {label} reflected {name} 실패: {ex.GetType().Name}:{ex.Message}");
                    }
                }
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} method scan 실패: {ex.GetType().Name}:{ex.Message}");
            }

            DentalLogger.Log($"Composite2SplitLine2 - {label} Rebuild 계열 메서드 미지원");
            return false;
        }

        private static bool TryInvokeNamedMethod(object target, string methodName, object[] args, string label)
        {
            try
            {
                target.GetType().InvokeMember(
                    ...
                    args,
                    CultureInfo.InvariantCulture);
                string argText = args == null || args.Length == 0
                    ? "()"
                    : "(" + string.Join(",", Array.ConvertAll(args, a => Convert.ToString(a, CultureInfo.InvariantCulture))) + ")";
                DentalLogger.Log($"Composite2SplitLine2 - {label} {methodName}{argText} 호출 성공");
                return true;
            }
            catch
            {
        ...
        private static object TryGetOperationTechnology(object operation)
        {
            if (operation == null)
            {
                return null;
            ...
            foreach (string propertyName in propertyNames)
            {
                try
                {
                    object tech = operation.GetType().InvokeMember(
                        propertyName,
                        BindingFlags.GetProperty,
                        null,
                        operation,
        ...
        private static void TryCommitCompositePassPercentsViaReflection(
            object tech,
            double firstPassPercent,
            double lastPassPercent,
            string label)
        {
            ...
            try
            {
                ...
                tech.GetType().InvokeMember(
                    ...
                    CultureInfo.InvariantCulture);
                DentalLogger.Log(
                    $"Composite2SplitLine2 - {label} PassPercent reflection 재커밋: nudgeFirst={nudgeFirst.ToString("F4", CultureInfo.InvariantCulture)} -> First={first.ToString("F4", CultureInfo.InvariantCulture)}, Last={last.ToString("F4", CultureInfo.InvariantCulture)}");
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"Composite2SplitLine2 - {label} PassPercent reflection 재커밋 실패: {ex.GetType().Name}:{ex.Message}");
            }
        }
        ...
        /// </summary>
        private static double ClampFaceRightXBelowSplitline2(double requestedFaceRightX, out double splitline2Used, out bool clampApplied)
        {
            splitline2Used = double.NaN;
            clampApplied = false;

            if (!TryGetThreeStageSplitConfig(out _, out double splitline2, out _, out _))
            {
                return requestedFaceRightX;
            }

            splitline2Used = splitline2;
            double maxFaceRightBySplitline2 = splitline2 - FrontFaceSplitline2NoCrossMarginMm;
            if (requestedFaceRightX >= maxFaceRightBySplitline2)
            {
                clampApplied = true;
                return maxFaceRightBySplitline2;
            }

            return requestedFaceRightX;
        }

        private static void SetFaceBottomZLimitFromRightX(TechLatheMoldParallelPlanes faceOp, double faceRightX)
        {
            if (RL == 1.0)
            {
                faceOp.BottomZLimit = -faceRightX;
            }
            else if (RL == 2.0)
            {
                faceOp.BottomZLimit = faceRightX;
        ...
        private static bool TryApplyFaceRightEndGuard(TechLatheMoldParallelPlanes faceOp, string context)
        {
            ...
            try
            {
                ...
                if (Math.Abs(adjustedFaceRightX - currentFaceRightX) < 1e-9)
                {
                    DentalLogger.Log($"FaceRoughGuard[{context}] - 유지 (보정값 동일, RoughA.RightX={roughARightX:F3}, Face.RightX={currentFaceRightX:F3}, splitX={splitXUsed:F3}, Splitline2={splitline2Used:F3})");
                    return false;
                }

                double oldBottom = faceOp.BottomZLimit;
                SetFaceBottomZLimitFromRightX(faceOp, adjustedFaceRightX);

                DentalLogger.Log($"FaceRoughGuard[{context}] - 보정 적용 (RoughA.RightX={roughARightX:F3}, Face.RightX:{currentFaceRightX:F3}->{adjustedFaceRightX:F3}, gap:{currentGap:F3}, RoughGuard={roughGuardApplied}, Splitline2Clamp={splitline2ClampApplied}, BottomZLimit:{oldBottom:F3}->{faceOp.BottomZLimit:F3}, splitX={splitXUsed:F3}, Splitline2={splitline2Used:F3})");
                return true;
            }
            catch (Exception ex)
            {
        ...
        private static void AddSplitOp(string region, string angleLabel, int boundaryKey, FreeFormFeature freeFormFeature, string prcFile, TechnologyUtility technologyUtility, bool applyBoundary)
        {
            ...
            if (tech.Length > 1 && tech[1] is TechLatheMoldZLevel zlevel)
            {
                ...
                else
                {
                    ...
                    {
                        DentalLogger.Log($"RoughFreeFromMillSplitAB - Region:{region} Angle:{angleLabel} ZLevel 경계 프로파일 미적용(applyBoundary=false)");
                    }
...
#pragma warning restore CS0162
        ...
        private static FeatureChain EnsureRectBoundaryXZ(string name, double x1, double x2, double zTop, double zBottom)
        {
            ...
            {
                int existingKey = SafeParseKey(Convert.ToString(existing.Key, CultureInfo.InvariantCulture));
                DentalLogger.Log($"EnsureRectBoundaryXZ({name}) - 기존 체인 재사용 (Key:{existingKey})");
                return existing;
            }
            ...
            try
            {
                ...
                DentalLogger.Log($"EnsureRectBoundaryXZ({targetName}) 생성 - X[{xLeft:0.###}~{xRight:0.###}], Z[{zLower:0.###}~{zUpper:0.###}]");
                return fc;
            }
            catch (Exception ex)
            {
                DentalLogger.Log($"EnsureRectBoundaryXZ({name}) 실패: {ex.GetType().Name}:{ex.Message}");
                return null;
            }
        }
    }
