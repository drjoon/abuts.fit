...
5) 헥스 단면 각도를 one-shot 계산으로 Z축 회전 정렬 (Right(ZY) 뷰에서 헥스 중심이 면)
6) 방향/회전 보정 후 target_diameter 단면을 다시 Z=0으로 재정렬
...
def align_mesh_to_origin(mesh, target_diameter=None, implant_profile=None):
    ...
    # 5) 헥스 각도 정렬: Right(ZY) 뷰에서 헥스 중심이 '면'이 되도록 Z축 회전
    ok_hex, hex_msg = _align_hex_angle_for_right_view(mesh)
    if not ok_hex:
        _log("{} (skip hex-angle alignment)".format(hex_msg))

    # 6) 2차 Z 재정렬(요청사항): 방향/회전 보정 후 target_diameter 단면을 다시 Z=0으로 이동
