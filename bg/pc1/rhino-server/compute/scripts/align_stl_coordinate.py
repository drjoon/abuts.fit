...
2) 내부 스크류 홀(원기둥) 3개 단면 중심으로 축을 추정해 Z축으로 미세 회전 + XY 평행이동
3) 커넥션 외부 직경(target_diameter) 단면의 Z를 찾아 Z=0으로 평행이동
4) +Z 방향을 상단(넓은 쪽)으로 강제
5) 방향 보정 후 target_diameter 단면을 다시 Z=0으로 재정렬
6) 헥스 6면 각도 기준으로 Z축 회전 정렬 (post-Z2 1회)
7) 회전 후 `residual_to_X_deg`를 재측정해 성공 메시지/로그로 노출
...
def _fit_hole_axis_three_sections(mesh, target_diameter=None, section_cache=None):
    """
...
def _fit_hole_axis_preferred(
    ...
    """
    우선순위:
    1) 3개 단면 중심 기반 축 추정 (원기둥 가정)
    2) 기존 다중 샘플 밴드 회귀 방식 fallback
    """
...
def align_mesh_to_origin(mesh, target_diameter=None, implant_profile=None):
    ...
    # 3) 1차 Z 정렬: target_diameter 단면을 Z=0으로 이동
    ...
    # 4) 방향 보정: 상단(+Z)이 하단(-Z)보다 단면이 넓은 방향으로 강제
    _enforce_wider_end_points_to_positive_z(mesh)

    # 5) 2차 Z 재정렬(요청사항): 방향 보정 후 target_diameter 단면을 다시 Z=0으로 이동
