...
def align_mesh_to_origin(mesh, target_diameter=3.33):
    ...
    _log("Found Z height: {:.3f}mm".format(z_target))
    ...
    center_x, center_y, radius = result
    _log(
        "Circle center: ({:.3f}, {:.3f}), radius: {:.3f}mm".format(
            center_x, center_y, radius
        )
    )
    _log("Calculated diameter: {:.3f}mm".format(radius * 2))

    # 3단계: 평행 이동 벡터 계산
    translation = rg.Vector3d(-center_x, -center_y, -z_target)
    _log(
        "Translation: ({:.3f}, {:.3f}, {:.3f})".format(
...
def main(input_path, output_path):
    ...
    success, message, _translation = align_mesh_to_origin(mesh)
