...
6) 홀축 미세회전 이후 target_diameter 단면을 다시 Z=0으로 재정렬(Z-3)
7) 헥스 6면 각도는 측정/기록만 수행 (실제 mesh Z회전 적용 금지)
8) 필요 시 가상 회전(복제 mesh)으로 잔차를 재측정해 telemetry/로그로 노출
...
# 헥스 residual은 파이프라인 실패 조건이 아니다.
# 잔차가 높아도 경고만 남기고 진행한다. (strict 실패 경로 제거)
...
def _align_hex_angle_for_right_view(mesh):
    ...
    virtual_applied_deg = -phase_mod_deg
    initial_residual_deg = float(residual.get("residual_deg", 999.0))

    # 가상 검증: 복제 mesh에만 보정량을 적용해 잔차가 얼마나 줄어드는지 확인.
    # 원본 mesh는 끝까지 불변이어야 한다.
    virtual_residual = None
    try:
        probe = mesh.DuplicateMesh()
        if probe is not None and _apply_z_rotation_deg(probe, virtual_applied_deg):
            virtual_residual = _measure_hex_residual_to_x_deg(probe)
    except Exception:
        virtual_residual = None

    _log(
        "Hex telemetry-only: before_to_X={:.6f}deg virtual_applied={:.6f}deg residual_to_X={:.6f}deg method={} samples={} target<={:.6f}".format(
            phase_mod_deg,
            virtual_applied_deg,
    ...
    if initial_residual_deg > float(HEX_RESIDUAL_TARGET_DEG):
        _log(
            "Hex residual warning (telemetry-only): {:.6f} > {:.6f}".format(
                initial_residual_deg,
                float(HEX_RESIDUAL_TARGET_DEG),
            )
        )
    ...
    if isinstance(virtual_residual, dict):
        telemetry["virtual_residual_deg"] = float(virtual_residual.get("residual_deg", 999.0))
        telemetry["virtual_method"] = virtual_residual.get("method", "unknown")
...
def _enforce_longer_side_to_plus_z(mesh, eps=0.03):
    """
    방향 강제 규칙:
    z=0 기준으로 +Z 방향 모델 길이와 -Z 방향 모델 길이를 비교해,
    더 긴 쪽이 +Z가 되도록 보정한다.

    - upper_len = bbox.Max.Z
    - lower_len = -bbox.Min.Z
    """
    bbox = mesh.GetBoundingBox(True)

    upper_len = max(0.0, float(bbox.Max.Z))
    lower_len = max(0.0, -float(bbox.Min.Z))

    _log(
        "Length-probe orientation: upper_len(+Z)={:.4f} lower_len(-Z)={:.4f} (longer should be +Z)".format(
            upper_len,
            lower_len,
        )
    )
    ...
    # 동률/근접 노이즈는 뒤집지 않는다.
    if upper_len + float(eps) < lower_len:
        rot = rg.Transform.Rotation(
            math.pi,
            rg.Vector3d(1, 0, 0),
            rg.Point3d(0, 0, 0),
        ...
        if mesh.Transform(rot):
            _log("Flipped mesh 180° around X: enforced longer side to +Z")
            return True
        _log_error("Failed to flip mesh for length-probe orientation")
        return False

    _log("Length-probe rule satisfied (or near-tie): orientation kept")
    return False
...
def align_mesh_to_origin(mesh, target_diameter=None, implant_profile=None):
    ...
    # 3) 2차 Z 재정렬: 다중해 방지를 위해 -Z(임플란트/포스트) 측 직경 해를 우선 사용
    ...
    # 4) 방향 보정(사용자 지정):
    #    Z-2 기준 원점에서 +Z/-Z 길이를 비교해 긴 쪽이 +Z가 되도록 강제
    flipped = _enforce_longer_side_to_plus_z(mesh)

    #    flip이 발생하면 target_diameter 단면을 다시 Z=0으로 재고정
    if flipped:
        ok_z2b, z2b_msg, translation_2b = _translate_z_to_target_diameter_zero(
            mesh,
            resolved_diameter,
            stage_label="[Z-2b]",
            prefer_negative_side=True,
        )
        if not ok_z2b or translation_2b is None:
            return (False, z2b_msg or "Z-2b alignment failed", None)
        total_translation = rg.Vector3d(
            total_translation.X + translation_2b.X,
            total_translation.Y + translation_2b.Y,
            total_translation.Z + translation_2b.Z,
        )
    ...
    # 7) 헥스 각도 측정/기록(post-Z3 1회): 실제 mesh Z회전은 적용하지 않는다.
    #    필요 시 _align_hex_angle_for_right_view 내부에서 복제 mesh로만 가상 검증한다.
    ok_hex, hex_msg, hex_alignment = _align_hex_angle_for_right_view(mesh)
    if not ok_hex:
        _log("{} (hex telemetry unavailable @post-Z3)".format(hex_msg))

    # 8) 최종 잔차 재측정(원본 mesh 기준; 본 세션 정책상 헥스 Z회전 미적용 상태)
    ...
    if residual_deg > float(HEX_RESIDUAL_TARGET_DEG):
        warn_msg = (
            ...
            "apply-aligned-mesh-with-warning"
        ).format(
            residual_deg,
            float(HEX_RESIDUAL_TARGET_DEG),
            residual.get("method", "unknown"),
