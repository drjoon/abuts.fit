...
using Acrodent.EspritAddIns.ESPRIT2025AddinProject;
using Esprit;
...
namespace DentalAddin
{
 ...
 internal sealed class MainModule
 {
	...
	static MainModule()
	{
		InitDefaultsFromProcessConfig();
	}

	private static void InitDefaultsFromProcessConfig()
	{
		try
		{
			PrcDirectory = string.IsNullOrWhiteSpace(ProcessConfig.TechRootDirectory) ? PrcDirectory : ProcessConfig.TechRootDirectory;
			DefaultXmlFileName = Path.Combine(PrcDirectory, "Viles", "DefaultPath", "Tech_Default_Path.xml");
			...
			Chamfer = ProcessConfig.DefaultChamfer;
			DownZ = ProcessConfig.DefaultDownZ;
			TurningDepth = ProcessConfig.DefaultTurningDepth;
			MillingDepth = ProcessConfig.DefaultMillingDepth;
			TurningExtend = ProcessConfig.DefaultTurningExtend;

			PrcFilePath[1] = BuildPrcPath(ProcessConfig.TurningSubfolder, ProcessConfig.TurningProcessFile);
			PrcFilePath[2] = BuildPrcPath(ProcessConfig.ReverseTurningSubfolder, ProcessConfig.ReverseTurningProcessFile);
			PrcFilePath[3] = BuildPrcPath(ProcessConfig.RoughSubfolder, ProcessConfig.RoughMillingProcessFile);
			PrcFilePath[4] = BuildPrcPath(ProcessConfig.FaceHoleSubfolder, ProcessConfig.FaceHoleProcessFile);
			PrcFilePath[5] = BuildPrcPath(ProcessConfig.O180Subfolder, ProcessConfig.O180BallMillingProcessFile);
			PrcFilePath[6] = BuildPrcPath(ProcessConfig.O90_270Subfolder, ProcessConfig.O90_270BallMillingProcessFile);
			PrcFilePath[7] = BuildPrcPath(ProcessConfig.O90_270Subfolder, ProcessConfig.AdditionalProcessFile);
			PrcFilePath[8] = BuildPrcPath(ProcessConfig.ConnectionSubfolder, ProcessConfig.ConnectionProcessFile);
			PrcFilePath[9] = BuildPrcPath(ProcessConfig.SemiRoughSubfolder, ProcessConfig.SemiRoughMillingProcessFile);
			PrcFilePath[10] = BuildPrcPath(ProcessConfig.CompositeSubfolder, ProcessConfig.CompositeProcessFile);

			var markPath = BuildPrcPath(ProcessConfig.FaceSubfolder, ProcessConfig.MarkProcessFile);
			if (string.IsNullOrWhiteSpace(markPath))
			{
				markPath = PrcFilePath[10];
			}
			PrcFilePath[11] = markPath;
			PrcFilePath[12] = BuildPrcPath(ProcessConfig.FaceSubfolder, ProcessConfig.MarkTextProcessFile);
		}
		catch
		{
	...
	private static string BuildPrcPath(string subfolder, string fileName)
	{
		if (string.IsNullOrWhiteSpace(fileName))
		{
			return string.Empty;
		}
		var root = string.IsNullOrWhiteSpace(ProcessConfig.TechRootDirectory) ? PrcDirectory : ProcessConfig.TechRootDirectory;
		return Path.Combine(root ?? string.Empty, subfolder ?? string.Empty, fileName);
	}
	...
	/// <summary>
	/// STL 파일을 현재 문서에 병합하고 DentalPipeline을 실행한다.
	/// Connect.cs에서 호출된다.
	/// </summary>
	public static void ProcessStlFile(string stlFilePath)
	{
		try
		{
			...
			try
			{
				int materialDiameter = ChooseMaterialDiameter(Conversions.ToDouble(PatientContext.MaxDiameter));
				string baseDir = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
				var templateDir = Path.Combine(baseDir ?? string.Empty, "Templates");
				var templatePath = Path.Combine(templateDir, $"Hanwha_D{materialDiameter}.est");
				Trace.WriteLine($"[MainModule] Template open start: {templatePath}");
				if (File.Exists(templatePath) && EspritApp != null)
				{
					try
					{
						Document = EspritApp.Document;
						Document?.MergeFile(templatePath, RuntimeHelpers.GetObjectValue(Missing.Value));
						Trace.WriteLine($"[MainModule] Template merged: {templatePath}, ActiveDoc={Document?.Name}");
					}
					catch (Exception exOpen)
					{
						Trace.WriteLine($"[MainModule] Template merge failed: {exOpen.Message}");
					}
				}
				else
				{
			...
			{
				Trace.WriteLine($"[MainModule] Template load error: {ex.Message}");
			}

			// PatientContext에서 전달된 가공 파라미터를 NumData/NumCombobox로 반영
			if (PatientContext.NumData != null)
			{
				NumData = PatientContext.NumData;
			}
			if (PatientContext.NumCombobox != null)
			{
				NumCombobox = PatientContext.NumCombobox;
			}

			// 기본 플래그를 ProcessConfig 기준으로 재적용
			SpindleSide = ProcessConfig.DefaultSpindleSide;
			ReverseOn = ProcessConfig.DefaultReverseOn;
			RoughType = ProcessConfig.DefaultRoughType;
			...
			try
			{
				if (Document != null && !string.IsNullOrWhiteSpace(stlFilePath))
				{
					Document.MergeFile(stlFilePath, RuntimeHelpers.GetObjectValue(Missing.Value));
				}
			}
			catch (Exception ex)
			{
				Trace.WriteLine($"[MainModule] STL merge failed: {ex.Message}");
			}
		...
		{
			Trace.WriteLine($"[MainModule] Error in ProcessStlFile: {ex.Message}");
			throw;
		}
	}
	...
	public static void TurningOp()
	{
		...
		checked
		{
			...
			do
			{
				if (array[i] != null)
				{
					Document.Operations.Add(techLatheContour, array[i], RuntimeHelpers.GetObjectValue(Missing.Value));
					if (ReverseOn)
					{
						Document.Operations.Add(techLatheContour2, array[i], RuntimeHelpers.GetObjectValue(Missing.Value));
					}
				}
				i++;
			}
			...
			try
			{
				...
				checked
				{
					...
					case 2470:
						...
						IL_01f4:
						num2 = 29;
						Document.Operations.Add(techLatheMoldRoughing, array[1], RuntimeHelpers.GetObjectValue(Missing.Value));
						goto IL_0216;
						IL_0216:
						num2 = 30;
						if (array2.Count() > 1)
						...
						IL_025b:
						num2 = 34;
						Document.Operations.Add(techLatheMoldZLevel, array[1], RuntimeHelpers.GetObjectValue(Missing.Value));
						goto IL_027d;
						IL_027d:
						num2 = 35;
						array2 = (ITechnology[])technologyUtility.OpenProcess(file);
						...
						IL_02c8:
						num2 = 39;
						Document.Operations.Add(techLatheMoldRoughing, array[2], RuntimeHelpers.GetObjectValue(Missing.Value));
						goto IL_02ea;
						IL_02ea:
						num2 = 40;
						if (array2.Count() > 1)
						...
						IL_0332:
						num2 = 44;
						Document.Operations.Add(techLatheMoldZLevel, array[2], RuntimeHelpers.GetObjectValue(Missing.Value));
						goto end_IL_0000;
						IL_00c9:
						num2 = 11;
						...
						IL_0764:
						num2 = 99;
						Document.Operations.Add(techLatheMoldRoughing, array[3], RuntimeHelpers.GetObjectValue(Missing.Value));
						goto IL_0786;
						IL_0786:
						num2 = 100;
						if (array2.Count() > 1)
					...
					num2 = 104;
					Document.Operations.Add(techLatheMoldZLevel, array[3], RuntimeHelpers.GetObjectValue(Missing.Value));
				}
				end_IL_0000:;
			}
			catch (Exception ex) when (num3 != 0 && num == 0)
