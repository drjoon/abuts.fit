    ...
    public class Connect : DPTechnology.AnnexLibraries.EspritAddIn
    {
        ...
        private sealed class AddInStatusForm : Form
        {
            ...
            public void RefreshFiles()
            {
                ...
                {
                    Trace.WriteLine($"Filled STL 목록을 불러올 수 없습니다: {ex.Message} (MessageBox suppressed)");
                }
            }
            ...
            private async void MergeSelected()
            {
                ...
                if (doc == null)
                {
                    _log?.Invoke("[Addin] 현재 열린 ESPRIT 문서를 찾을 수 없습니다. (MessageBox suppress)");
                    return;
                }
                ...
                {
                    _log?.Invoke($"[Addin] 병합 실패: {ex.Message} (MessageBox suppress)");
                    System.Diagnostics.Trace.WriteLine($"MergeSelected failed: {ex}");
                }
            }
        }
        ...
        public void _ConnectionManager_AddInConnect(Esprit.Application espritApplication)
        {
            ...
            if (_repeatProcess == null)
            {
                try
                {
                    var addonDir = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                    _repeatProcess = new RepeatProcess(_espApp, addonDir);
                    _repeatProcess.Run();
                }
                catch (Exception ex)
                {
        ...
        private void ShowFilledBrowser()
        {
            ...
            {
                Trace.WriteLine($"Filled STL Browser를 열 수 없습니다: {ex.Message} (MessageBox suppressed)");
            }
        }
