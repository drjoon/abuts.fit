...
`CncContinuousMachining` 클래스는 **단일 슬롯(O4000)** 을 사용하여 연속 가공을 수행합니다.
...
1. 장비 상태 확인 (Idle 대기)
2. O4000 삭제 후 프로그램 업로드
3. O4000 활성화 및 Start
4. 생산 수량 기록

#### B. 반복

- 가공 완료 감지 후 다음 작업에 대해 A 단계 반복
...
1. **상태 폴링**: 3초 간격으로 가공 완료 감지를 위한 최소 신호(busy/productCount)만 확인
2. **완료 조건**:
...
- 가공 중에는 프로그램 삭제/업로드/활성화를 하지 않음
- 모든 프로그램 교체는 Idle 상태에서만 수행
- 프로그램 번호는 업로드 시 O4000으로 강제될 수 있음
