...
private static async Task PreloadNextJob(string machineId, MachineState state)
{
    ...
    try
    {
        ...
        if (uploaded)
        {
            ...
            CncJobQueue.Pop(machineId);
            Console.WriteLine("[CncMachining] preload success machine={0} slot=O{1}", machineId, state.NextSlot);
...
Console.WriteLine("[CncMachining] delete existing failed machine={0} slot=O{1} err={2}", machineId, state.CurrentSlot, delErr);
...
Console.WriteLine("[CncMachining] delete prev skipped machine={0} slot=O{1}", machineId, state.CurrentSlot);
